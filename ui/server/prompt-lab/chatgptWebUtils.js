...
export function parseChatGptSessionCookieHeader(rawCookieHeader) {
  const input = String(rawCookieHeader ?? '').trim().replace(/^cookie:\s*/i, '')
  if (!input) return []
  ...
  for (const part of parts) {
    ...
    if (!name || SET_COOKIE_ATTRIBUTE_NAMES.has(lowerName) || seenNames.has(lowerName)) {
      continue
    }
...
export function normalizeChatGptWebCitationRefs(rawCandidates, responseText) {
  ...
  for (const candidate of candidates) {
    ...
    const bounds = assignCitationBounds(responseText, normalized.anchorText, cursor)
