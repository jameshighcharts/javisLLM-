...
export function aggregateCitationSources(inputs: CitationSourceInput[]): CitationSourceStat[] {
  ...
  inputs.forEach((input, index) => {
    ...
    refs.forEach((ref) => {
      ...
      const normalizedTitle = typeof ref.title === 'string' ? ref.title.trim() : ''
      if (
        normalizedTitle &&
  ...
  return [...buckets.values()]
    .map((bucket) => ({
      ...
      providers: [...bucket.providers.values()].sort((left, right) => left.localeCompare(right)),
    }))
    .sort((left, right) => {
