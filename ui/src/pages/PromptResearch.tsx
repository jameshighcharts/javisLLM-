...
export default function PromptResearch() {
  ...
  const createCohortMutation = useMutation({
    mutationFn: (payload: {
      ...
      targetPp?: number
      targetWeeks?: number
    }) => api.createPromptResearchCohort(payload, normalizedTriggerToken || undefined),
  ...
  const optimizationByStatus = useMemo(() => {
    const grouped: Record<ContentGapStatus, Awaited<ReturnType<typeof api.researchGaps>>> = {
      backlog: [],
      in_progress: [],
      published: [],
      verify: [],
    ...
    <div className="max-w-[1360px] space-y-5">
      ...
      >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <input
                ...
                onChange={(event) => setNewCohortTargetWeeks(event.target.value)}
                className="w-full px-3 py-2 rounded-lg text-sm"
                style={{ border: '1px solid #DDD0BC', background: '#FFFFFF', color: '#2A3A2C' }}
            ...
            <button
              ...
              style={{
                background:
                  hasTriggerToken && !createCohortMutation.isPending ? '#2A6032' : '#E8E0D2',
                color:
                  hasTriggerToken && !createCohortMutation.isPending ? '#FFFFFF' : '#9AAE9C',
                border: `1px solid ${
                  hasTriggerToken && !createCohortMutation.isPending ? '#1E4A26' : '#DDD0BC'
                }`,
                cursor:
                  hasTriggerToken && !createCohortMutation.isPending ? 'pointer' : 'not-allowed',
              }}
            >
            ...
            {!hasTriggerToken && (
              <p className="text-xs" style={{ color: '#9AAE9C' }}>
                Trigger token required for cohort writes. Set it on the Runs page.
              </p>
            )}
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold" style={{ color: '#5D7260' }}>
                Active cohort
              </span>
              <select
              ...
              >
                {(cohortsQuery.data ?? []).map((cohort: PromptResearchCohort) => (
            ...
            {progressQuery.isLoading ? (
              <div className="h-16 rounded animate-pulse" style={{ background: '#F2EDE6' }} />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div className="rounded-md px-2 py-2" style={{ background: '#EEF5EF', color: '#2C5D30' }}>
                  Baseline {progressQuery.data.baselineRate?.toFixed(2) ?? '—'}%
                </div>
                <div className="rounded-md px-2 py-2" style={{ background: '#F3F7FB', color: '#2F5B84' }}>
                  Current {progressQuery.data.currentRate?.toFixed(2) ?? '—'}%
                </div>
                <div className="rounded-md px-2 py-2" style={{ background: '#FFF6E8', color: '#A66619' }}>
                  Uplift {progressQuery.data.upliftPp?.toFixed(2) ?? '—'}pp
                </div>
                <div className="rounded-md px-2 py-2" style={{ background: '#F4F1FF', color: '#6A4EB0' }}>
                  Due{' '}
      ...
      >
        ...
        {optimizationGapsQuery.isLoading ? (
          <div className="h-20 rounded animate-pulse" style={{ background: '#F2EDE6' }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3">
            {OPTIMIZATION_STATUSES.map((status) => (
              <div
                ...
                style={{ background: '#FDFCF8', border: '1px solid #EDE8E0' }}
              >
                <div className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#7A8E7C' }}>
                ...
                {optimizationByStatus[status].slice(0, 8).map((gap) => (
                  ...
                  >
                    <div className="text-xs font-medium" style={{ color: '#2A3A2C' }}>
                      {gap.topicLabel}
                    </div>
                    <div className="text-[11px] tabular-nums" style={{ color: '#7A8E7C' }}>
                      Score {(gap.compositeScore * 100).toFixed(1)}% · Evidence {gap.evidenceCount}
                    </div>
                    <select
                      disabled={!hasTriggerToken || updateOptimizationStatusMutation.isPending}
                      value={gap.status}
                      onChange={(event) =>
