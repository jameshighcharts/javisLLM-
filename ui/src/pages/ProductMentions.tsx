...
const PRODUCT_ORDER: ProductKey[] = ['core', 'stock', 'maps', 'gantt', 'dashboards', 'grid']
...
const PRODUCT_META: Record<ProductKey, ProductMeta> = {
  core: {
    ...
    surface: '#EEF6EF',
    border: '#C9DDCA',
    summary: 'Generic interactive charting for line, bar, area, pie, scatter, and mixed-series use cases.',
    trigger: 'Lead with Core when the query is broad charting or data visualization without a stronger domain signal.',
    expandWith: 'Expand only when the prompt also asks for a shell like dashboards, grids, maps, or planning views.',
  },
  stock: {
    ...
    surface: '#EEF6FB',
    border: '#C6DDED',
    summary: 'Financial and market-focused charting for candlestick, OHLC, navigator, range selector, and indicators.',
    trigger: 'Use Stock when the query says financial, trading, stock, candlestick, OHLC, or technical indicators.',
    expandWith: 'Pair with Dashboards or Grid when the prompt is really describing a trading workspace.',
  },
  maps: {
    ...
    surface: '#EFF5FF',
    border: '#C9D9F1',
    summary: 'Geospatial visualization for map charts, choropleths, GeoJSON layers, and regional analytics.',
    trigger: 'Use Maps when geography is explicit: map charts, choropleths, GeoJSON, territory views, or location data.',
    expandWith: 'Layer in Dashboards and Grid when maps are part of a broader analytics surface.',
  },
  gantt: {
    ...
    surface: '#FCF2EE',
    border: '#EACDBF',
    summary: 'Scheduling and planning for project timelines, milestones, dependencies, and resource management.',
    trigger: 'Use Gantt when the query says gantt, project timeline, scheduling, roadmap, dependency, or resource planning.',
    expandWith: 'Expand to Dashboards when the prompt is asking for a portfolio or PMO-style shell.',
  },
  dashboards: {
    ...
    surface: '#FBF6E7',
    border: '#E6D9AC',
    summary: 'Multi-widget analytics surfaces with synchronized charts, KPIs, filters, and coordinated layouts.',
    trigger: 'Use Dashboards when the query is about analytics dashboards, KPI workspaces, multi-panel views, or embedded analytics.',
    expandWith: 'Dashboards usually needs Core for charts and often Grid for table-heavy views.',
  },
  grid: {
    ...
    surface: '#F5F1FB',
    border: '#D6C8EA',
    summary: 'Tabular UI for data grids, spreadsheet-like interaction, sorting, filtering, and dense row/column work.',
    trigger: 'Use Grid when the query is clearly about a data grid, spreadsheet feel, or heavy table interaction.',
    expandWith: 'Grid becomes stronger when paired with Dashboards and the matching chart product.',
  },
}
...
const PRODUCT_RULES: Record<ProductKey, ProductRule[]> = {
  core: [
    ...
    { label: 'line or bar charts', pattern: /\b(line|bar|area|pie|scatter|column) chart\b/i },
  ],
  stock: [
    ...
    { label: 'technical indicators', pattern: /\b(indicator|indicators|technical analysis|macd|rsi)\b/i },
  ],
  maps: [
    { label: 'maps', pattern: /\bmap(?:s)?\b/i },
    { label: 'choropleth', pattern: /\bchoropleth\b/i },
    { label: 'geojson', pattern: /\bgeojson\b/i },
    { label: 'geospatial', pattern: /\b(geospatial|geographic|location analytics|territory)\b/i },
  ...
  gantt: [
    ...
    { label: 'scheduling', pattern: /\b(schedule|scheduling|resource planning|milestone)\b/i },
  ],
  dashboards: [
    ...
    { label: 'widgets and filters', pattern: /\b(widget|widgets|scorecard|scorecards|multi-panel)\b/i },
  ],
  grid: [
    ...
    { label: 'tabular', pattern: /\b(tabular|table component|row grouping|column pinning)\b/i },
  ],
}
...
const SUGGESTED_QUERIES: SuggestedQuery[] = [
  {
    ...
    rationale: 'Broad charting is the default Core motion and should exist in the live prompt set.',
  },
  {
    id: 'core-react',
    product: 'core',
    ...
    rationale: 'React chart-library intent should be tracked separately from generic JavaScript charting.',
  },
  {
    id: 'stock-javascript',
    product: 'stock',
    ...
    rationale: 'Finance language should force the answer toward Highcharts Stock instead of plain Highcharts.',
  },
  {
    id: 'stock-react',
    product: 'stock',
    ...
    rationale: 'React financial prompts are important because wrappers often bias answers toward React-native competitors.',
  },
  {
    id: 'maps-javascript',
    product: 'maps',
    ...
    rationale: 'Map-chart phrasing should surface Maps directly rather than generic charting libraries.',
  },
  {
    id: 'maps-react',
    product: 'maps',
    ...
    rationale: 'React map-chart prompts deserve direct coverage because they often pull in mapping-first rivals.',
  },
  {
    id: 'gantt-javascript',
    product: 'gantt',
    ...
    rationale: 'Planning and dependency queries should explicitly test whether Highcharts Gantt gets named.',
  },
  {
    id: 'gantt-react',
    product: 'gantt',
    ...
    rationale: 'React gantt prompts are a distinct buying path and should not be inferred from JavaScript-only coverage.',
  },
  {
    ...
    rationale: 'Dashboards should be mentioned as the shell when developers ask for analytics workspaces.',
  },
  {
    id: 'dashboards-react',
    product: 'dashboards',
    ...
    rationale: 'React dashboard intent deserves its own coverage because many answers default to app frameworks instead of products.',
  },
  {
    id: 'grid-javascript',
    product: 'grid',
    ...
    rationale: 'Grid needs its own prompts because table-heavy asks are not chart-library asks.',
  },
  {
    id: 'grid-react',
    product: 'grid',
    ...
    rationale: 'React data grid is a separate decision path and should be monitored directly.',
  },
]
...
function classifyFrameworks(prompt: PromptStatus): FrameworkTag[] {
  ...
  if (normalizedTags.includes('javascript') || /\bjavascript\b/.test(query) || /\bjs\b/.test(query)) {
...
function classifyProductMention(prompt: PromptStatus): Pick<
  ClassifiedPrompt,
...
> {
  ...
  const specializedMatches = SPECIALIZED_PRODUCTS.filter((product) => matches[product].length > 0)
  ...
  if (leadProduct === 'dashboards') {
    ...
    if (leadProduct === 'stock' || leadProduct === 'maps' || leadProduct === 'gantt') {
      supportingProducts.push('core')
    }
  ...
  const orderedSupportingProducts = PRODUCT_ORDER.filter(
    (product) => product !== leadProduct && supportingProducts.includes(product),
  )
...
function StatCard({
  ...
  return (
    <div
      ...
      style={{ background: '#FFFFFF', borderColor: '#DDD0BC', boxShadow: '0 12px 30px rgba(61,92,64,0.05)' }}
    >
      <div className="text-[11px] uppercase tracking-[0.14em]" style={{ color: '#8A9B8C' }}>
        {label}
      </div>
      <div className="mt-1 text-[28px] font-semibold tracking-tight" style={{ color: '#263628' }}>
        {value}
      </div>
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            {item.frameworks.map((framework) => (
              <FrameworkChip key={`${item.prompt.query}-${framework}`} framework={framework} />
            ))}
            <VisibilityBadge status={item.visibilityStatus} />
            <ConfidenceBadge confidence={item.confidence} />
          </div>
          <div className="text-base font-semibold tracking-tight" style={{ color: '#253325' }}>
            {item.prompt.query}
          </div>
        </div>
        ...
        <div
          ...
          style={{ background: leadMeta.surface, border: `1px solid ${leadMeta.border}` }}
        >
          <div className="text-[10px] uppercase tracking-[0.16em]" style={{ color: '#809282' }}>
            Lead product
          </div>
          <div className="text-xs font-semibold" style={{ color: leadMeta.accent }}>
            {leadMeta.shortName}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {item.supportingProducts.map((product) => (
          <ProductChip key={`${item.prompt.query}-${product}`} product={product} muted />
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        >
          <div className="text-[10px] uppercase tracking-[0.16em]" style={{ color: '#8A9B8C' }}>
            HC mention
          </div>
          <div className="mt-1 text-sm font-semibold" style={{ color: '#2B6031' }}>
        ...
        >
          ...
          <div className="mt-1 text-sm font-semibold" style={{ color: '#253325' }}>
            {item.prompt.runs}
          </div>
        </div>
        <div
        ...
        <div className="flex flex-wrap gap-2">
          {item.matchedRules.map((rule) => (
            <span
              ...
              className="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold"
              style={{ background: '#F5EFE5', color: '#7A6755', border: '1px solid #E5D6C2' }}
            >
              {rule}
            </span>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {item.prompt.tags.map((tag) => (
            <span
              ...
              className="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold"
              style={{ background: '#F4F2EC', color: '#6B746A', border: '1px solid #DDD0BC' }}
            >
              {tag}
            </span>
          ))}
...
function SuggestionBadge({ state }: { state: SuggestionState }) {
  const styles =
    state === 'missing'
      ? { background: '#FEF2F2', color: '#B42318', border: '#F4CACA', label: 'Missing' }
      : state === 'needs_runs'
        ? { background: '#FBF6E7', color: '#8C6B17', border: '#E6D9AC', label: 'Needs runs' }
        : { background: '#FDF5E8', color: '#9C5B3A', border: '#EACDBF', label: 'Needs lift' }
...
function LoadingCard() {
  return (
    ...
    >
      ...
      <div className="h-10 w-full rounded-2xl" style={{ background: '#F2EDE6' }} />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="h-28 rounded-2xl" style={{ background: '#F2EDE6' }} />
        ))}
      </div>
    </div>
  )
...
export default function ProductMentions() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => setSuggestionCopyMessage(''), 1800)
    return () => window.clearTimeout(timeoutId)
  ...
  const productCoverage = useMemo(() => {
    return PRODUCT_ORDER.map((product) => {
      const rows = classifiedPrompts.filter((item) => item.leadProduct === product)
      const trackedRows = rows.filter((item) => item.visibilityStatus === 'tracked')
      return {
        product,
        totalQueries: rows.length,
        trackedQueries: trackedRows.length,
        pausedQueries: rows.filter((item) => item.visibilityStatus === 'paused').length,
        avgHighchartsRate: average(trackedRows.map((item) => item.prompt.highchartsRatePct)),
        javascriptCount: rows.filter((item) => item.frameworks.includes('javascript')).length,
        reactCount: rows.filter((item) => item.frameworks.includes('react')).length,
        generalCount: rows.filter((item) => item.frameworks.includes('general')).length,
        bundleCount: rows.filter((item) => item.supportingProducts.length > 0).length,
  ...
  const filteredPrompts = useMemo(() => {
    return classifiedPrompts.filter((item) => {
      if (frameworkFilter !== 'all' && !item.frameworks.includes(frameworkFilter)) {
        return false
      }
  ...
  }, [classifiedPrompts, frameworkFilter, normalizedSearch, productFilter, statusFilter])

  const visibleCoverage = useMemo(() => {
    return PRODUCT_ORDER.map((product) => {
      const rows = filteredPrompts.filter((item) => item.leadProduct === product)
      const trackedRows = rows.filter((item) => item.visibilityStatus === 'tracked')
      return {
        product,
        totalQueries: rows.length,
  ...
  const coverageSuggestions = useMemo(() => {
    return SUGGESTED_QUERIES.map((suggestion) => {
      ...
      const trackedPrompts = matchingPrompts.filter((item) => item.visibilityStatus === 'tracked')
      ...
      .filter((suggestion) => suggestion.state !== 'needs_lift' || suggestion.avgHighchartsRate < 45)
      .sort((left, right) => {
  ...
  const visibleSuggestions = useMemo(() => {
    return coverageSuggestions.filter((suggestion) => {
      if (frameworkFilter !== 'all' && suggestion.framework !== frameworkFilter) {
        return false
      }
  ...
  const chartOptions: Highcharts.Options = useMemo(() => ({
    ...
    xAxis: {
      categories: visibleCoverage.map((row) => PRODUCT_META[row.product].shortName),
      lineColor: '#DDD0BC',
      tickColor: '#DDD0BC',
    ...
    tooltip: {
      ...
      formatter() {
        ...
        const row = visibleCoverage.find(
          (coverageRow) => PRODUCT_META[coverageRow.product].shortName === productLabel,
        )
        const lines = points
          .map(
            (point) =>
  ...
  async function copySuggestedQueries() {
    ...
    try {
      await navigator.clipboard.writeText(uniqueLines.join('\n'))
      setSuggestionCopyMessage(`Copied ${uniqueLines.length} suggested queries`)
    } catch (error) {
  ...
  if (dashboardQuery.isError) {
    return (
      <div
        ...
        style={{ background: '#FFF7F6', borderColor: '#F4CACA', color: '#8F2D2D' }}
      >
        ...
        <p className="mt-2 text-sm leading-relaxed">
          {String(dashboardQuery.error instanceof Error ? dashboardQuery.error.message : dashboardQuery.error)}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-[1480px] space-y-5">
      <section
        ...
        style={{
          background: 'linear-gradient(135deg, #FFFDF6 0%, #F8F1E6 44%, #EEF6EF 100%)',
          borderColor: '#DDD0BC',
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-3xl space-y-4">
              <div className="flex flex-wrap gap-2">
                <span
                  ...
                  style={{ background: '#FFFFFF', color: '#47604A', border: '1px solid #DDD0BC' }}
                >
                  <Sparkles size={14} />
                  Live prompt data
                </span>
              ...
              <div>
                <h2 className="text-3xl sm:text-[34px] font-semibold tracking-tight" style={{ color: '#223124' }}>
                  Highcharts product mention planner
                </h2>
                <p className="mt-2 max-w-2xl text-sm sm:text-[15px] leading-relaxed" style={{ color: '#5A6F5D' }}>
                  This board now classifies the live prompt inventory from benchmark data. Each prompt
                  is routed to the Highcharts product it implies, then measured against current
                  Highcharts mention performance.
                </p>
              </div>
            ...
            <div className="flex flex-col items-stretch gap-2 min-w-[220px]">
              ...
              <Link
                ...
                style={{ background: '#FFFFFF', color: '#3E5541', border: '1px solid #DDD0BC' }}
              >
                Open Query Lab
              </Link>
              <div className="h-5 text-center text-xs font-medium" style={{ color: '#6E8170' }}>
                {copyMessage}
              </div>
            </div>
          </div>
      ...
      <section
        className="rounded-[26px] border p-5"
        style={{ background: '#FFFFFF', borderColor: '#DDD0BC', boxShadow: '0 16px 40px rgba(42,58,44,0.05)' }}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold tracking-tight" style={{ color: '#243325' }}>
              Product routing rules with live coverage
            </h3>
            <p className="text-sm mt-1" style={{ color: '#6B7C6D' }}>
              Static rules define what each query should trigger. Live metrics show whether current prompts already cover that product well.
            </p>
          </div>
          <div className="text-xs font-medium" style={{ color: '#879888' }}>
            Coverage is based on the current prompt inventory, not hardcoded demo rows.
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-[0.16em]" style={{ color: '#7A8D7C' }}>
                      {meta.shortName}
                    </div>
                    <div className="text-base font-semibold tracking-tight" style={{ color: meta.accent }}>
                      {meta.name}
                    </div>
                  </div>
                  <div
                    className="h-10 w-10 rounded-2xl"
                    style={{ background: meta.accent, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.22)' }}
                  />
                </div>

                <p className="text-sm leading-relaxed" style={{ color: '#516452' }}>
                  {meta.summary}
                </p>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div
                    className="rounded-2xl border px-3 py-2"
                    style={{ background: '#FFFFFF', borderColor: 'rgba(255,255,255,0.58)' }}
                  >
                    <div className="text-[10px] uppercase tracking-[0.16em]" style={{ color: '#8A9B8C' }}>
                      Queries
                    </div>
                ...
                <div className="rounded-2xl px-3 py-3 text-sm leading-relaxed" style={{ background: '#FFFFFF', border: '1px solid rgba(255,255,255,0.58)' }}>
                ...
                <div className="rounded-2xl px-3 py-3 text-sm leading-relaxed" style={{ background: 'rgba(255,255,255,0.76)', border: '1px solid rgba(255,255,255,0.58)' }}>
                  <div className="font-semibold" style={{ color: '#334635' }}>
                    Expand with
                  </div>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div className="text-sm font-semibold" style={{ color: '#243325' }}>
                Filter live prompts
              </div>
            </div>
            ...
            <div className="mt-4 space-y-4">
              <label className="block">
                <span className="mb-2 block text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: '#8A9B8C' }}>
                  Search
                </span>
                <input
                  value={search}
                  ...
                  style={{ border: '1px solid #DDD0BC', background: '#FDFBF7', color: '#243325' }}
                />
              </label>

              <div>
                <div className="mb-2 text-xs font-semibold uppercase tracking-[0.14em]" style={{ color: '#8A9B8C' }}>
                  Framework
                </div>
                <div className="flex flex-wrap gap-2">
                  <FilterPill active={frameworkFilter === 'all'} label="All" onClick={() => startTransition(() => setFrameworkFilter('all'))} />
                  <FilterPill active={frameworkFilter === 'javascript'} label="JavaScript" onClick={() => startTransition(() => setFrameworkFilter('javascript'))} />
                  <FilterPill active={frameworkFilter === 'react'} label="React" onClick={() => startTransition(() => setFrameworkFilter('react'))} />
                  <FilterPill active={frameworkFilter === 'general'} label="General" onClick={() => startTransition(() => setFrameworkFilter('general'))} />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <FilterPill active={productFilter === 'all'} label="All products" onClick={() => startTransition(() => setProductFilter('all'))} />
                  {PRODUCT_ORDER.map((product) => (
                    <FilterPill
                      key={product}
                      active={productFilter === product}
                      label={PRODUCT_META[product].shortName}
                      onClick={() => startTransition(() => setProductFilter(product))}
                    />
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <FilterPill active={statusFilter === 'all'} label="All" onClick={() => startTransition(() => setStatusFilter('all'))} />
                  <FilterPill active={statusFilter === 'tracked'} label="Tracked" onClick={() => startTransition(() => setStatusFilter('tracked'))} />
                  <FilterPill active={statusFilter === 'awaiting_run'} label="Awaiting" onClick={() => startTransition(() => setStatusFilter('awaiting_run'))} />
                  <FilterPill active={statusFilter === 'paused'} label="Paused" onClick={() => startTransition(() => setStatusFilter('paused'))} />
                  <FilterPill active={statusFilter === 'deleted'} label="Deleted" onClick={() => startTransition(() => setStatusFilter('deleted'))} />
                </div>
              </div>
            </div>
          </div>
          ...
          >
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: '#6B7C6D' }}>
                Framework counts by lead product for the current filter. JavaScript and React counts can overlap on mixed prompts.
              </p>
            </div>
            <div className="mt-4">
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: '#6B7C6D' }}>
                {filteredPrompts.length} prompts visible, with {visibleTrackedCount} active tracked prompts in the current filter.
              </p>
            </div>
            <Link
              to="/prompts"
              className="inline-flex items-center rounded-full px-3 py-1.5 text-xs font-semibold"
              style={{ background: '#F5EFE5', color: '#5C6F5F', border: '1px solid #E5D6C2' }}
            >
              Open Prompts
            </Link>
          </div>
          ...
          {groupedPrompts.length === 0 ? (
            <div
              ...
              style={{ background: '#FBF7F0', borderColor: '#E7DAC8', color: '#6A7C6B' }}
            >
                ...
                <section key={group.product} className="space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <div className="text-lg font-semibold tracking-tight" style={{ color: '#243325' }}>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: '#6B7C6D' }}>
              These recommendations are generated from live coverage gaps, not a fixed page mockup.
            </p>
          </div>
          <div className="flex flex-col items-end gap-1">
            ...
            <div className="h-4 text-xs font-medium" style={{ color: '#6E8170' }}>
              {suggestionCopyMessage}
            </div>
          </div>
        </div>
        ...
        {visibleSuggestions.length === 0 ? (
          <div
            className="mt-4 rounded-2xl border px-4 py-5 text-sm"
            style={{ background: '#EEF6EF', borderColor: '#C9DDCA', color: '#356A3D' }}
          >
            No current gaps under this filter. The live dataset already covers these product and framework combinations reasonably well.
          </div>
        ) : (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="space-y-2">
                    ...
                    <div className="text-base font-semibold tracking-tight" style={{ color: '#243325' }}>
                      {suggestion.query}
                    </div>
                  </div>
                  <div
                    className="rounded-2xl px-3 py-2 text-right"
                    style={{ background: '#FFFFFF', border: '1px solid #E5D6C2' }}
                  >
                    <div className="text-[10px] uppercase tracking-[0.16em]" style={{ color: '#809282' }}>
                      Live coverage
                    </div>
                    <div className="text-xs font-semibold" style={{ color: '#253325' }}>
                      {suggestion.existingCount} queries
                    </div>
                  </div>
                </div>

                <p className="text-sm leading-relaxed" style={{ color: '#5E7260' }}>
                  {suggestion.rationale}
                </p>
