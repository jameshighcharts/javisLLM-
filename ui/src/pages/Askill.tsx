...
function ProviderPill({ provider }: { provider: string }) {
  ...
  const c = colors[provider] ?? { bg: '#F2EDE6', text: '#6C7C6E', border: '#DDD0BC' }
  return (
    <span
  ...
  if (rows.length === 0) {
    return (
      <div
        ...
        style={{ background: '#FDFCF8', borderColor: '#E4D8C8', color: '#8A9B8C' }}
      >
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[700px] border-collapse">
          <thead>
            <tr style={{ background: '#FDFCF8', borderBottom: '1px solid #EEE5D8' }}>
              <th
            ...
            {rows.map((row, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 align-top max-w-[520px]">
                  {row.title && row.title !== row.url && row.title !== row.host && (
                    <div
                  ...
                  <div className="text-[10px] mt-0.5" style={{ color: '#9AAE9C' }}>
                    {row.host}
                  </div>
                </td>
...
export default function Askill() {
  ...
  const query = useQuery({
    ...
    queryFn: () => api.askill({ runId: selectedRunId, providers: selectedProviders }),
    staleTime: 2 * 60 * 1000,
  })
        ...
        {availableRuns.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              style={{ background: '#FFFFFF', borderColor: '#DDD0BC', color: '#2A3A2C', outline: 'none' }}
              value={currentRunId ?? ''}
        ...
        <>
            ...
            {[
              ...
              >
                <div
                  ...
                  style={{ color: card.accent ? 'rgba(255,255,255,0.6)' : '#9AAE9C' }}
                >
                  {card.label}
                </div>
                <div
                ...
                <div
                  ...
                  style={{ color: card.accent ? 'rgba(255,255,255,0.5)' : '#B0A898' }}
                >
                  {card.sub}
                </div>
              </div>
          ...
          >
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <span className="text-xs font-medium mr-0.5" style={{ color: '#7A8E7C' }}>
                Provider
              </span>
              <button
            ...
            <div className="ml-auto text-xs font-medium" style={{ color: '#9AAE9C' }}>
              {filteredUrls.length.toLocaleString()} URL{filteredUrls.length !== 1 ? 's' : ''}
          ...
          {data.queries.length > 0 && (
            ...
            >
              ...
              >
                ...
                <span className="text-xs font-normal" style={{ color: '#9AAE9C' }}>
                  {data.queries.length} queries
                </span>
              </summary>
              <div className="border-t overflow-x-auto" style={{ borderColor: '#F2EDE6' }}>
                <table className="w-full min-w-[560px] border-collapse">
                  <thead>
                    <tr style={{ background: '#FDFCF8', borderBottom: '1px solid #EEE5D8' }}>
                      <th className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>Query</th>
                      <th className="px-4 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>Responses</th>
                      <th className="px-4 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>HC Mentions</th>
                      <th className="px-4 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>Mention %</th>
                      <th className="px-4 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>Citations</th>
                      <th className="px-4 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#8A9B8C' }}>URLs</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.queries.map((q, index) => (
                      <tr
                        key={q.queryId}
                        style={{
                          borderBottom: index < data.queries.length - 1 ? '1px solid #F2EDE6' : 'none',
                          background: index % 2 === 1 ? '#FDFCF8' : '#FFFFFF',
                        }}
                      >
                        <td className="px-4 py-2.5 text-sm max-w-xs" style={{ color: '#2A3A2C' }}>
                          {q.queryText}
                        </td>
                        <td className="px-4 py-2.5 text-right text-sm tabular-nums" style={{ color: '#5A7060' }}>
                          {q.responseCount}
                        </td>
                        <td className="px-4 py-2.5 text-right text-sm font-semibold tabular-nums" style={{ color: '#2A3A2C' }}>
                          {q.mentionCount}
                        </td>
                        <td className="px-4 py-2.5 text-right text-sm tabular-nums" style={{
                          color: q.mentionRatePct >= 75 ? '#2F6633' : q.mentionRatePct >= 40 ? '#8C6B17' : '#9C4A3A',
                          fontWeight: 600,
                        }}>
                          {q.mentionRatePct}%
                        </td>
