...
PACKAGE_SRC = Path(__file__).resolve().parents[2] / "packages" / "py" / "benchmark_core" / "src"
if str(PACKAGE_SRC) not in sys.path:
    sys.path.insert(0, str(PACKAGE_SRC))

from benchmark_core.queue import (  # noqa: E402
    build_entity_specs,
...
from worker._supabase_import import Client, create_client  # noqa: E402
...
class BenchmarkWorker:
    ...
    def process_queue_message(self, queue_message: Dict[str, Any]) -> None:
        ...
        if isinstance(job_id_raw, int):
            job_id = job_id_raw
        elif isinstance(job_id_raw, str) and job_id_raw.isdigit():
