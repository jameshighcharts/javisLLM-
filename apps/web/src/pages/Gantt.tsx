...
const EMPLOYEES: Employee[] = [
	{
		...
		name: "Elise Chester",
		role: "Web designer",
		avatarUrl: avatarDataUrl("EC", "#8FBB93", "#4A6B4E"),
	},
	{
		id: "e-freddie",
		name: "Freddie She",
		role: "Product designer",
		avatarUrl: avatarDataUrl("FS", "#D49880", "#A96A53"),
	},
	{
		id: "e-santi",
		name: "Santi Holmes",
		role: "Motion designer",
		avatarUrl: avatarDataUrl("SH", "#9DB88F", "#6C8855"),
	},
	{
		id: "e-aida",
		name: "Aida Keller",
		role: "UX researcher",
		avatarUrl: avatarDataUrl("AK", "#A89CB8", "#746682"),
	},
	{
		id: "e-tomas",
		name: "Tomas Graff",
		role: "Frontend engineer",
		avatarUrl: avatarDataUrl("TG", "#C8A87A", "#8F6E3B"),
	},
];
...
const INITIAL_TASKS: TimelineTask[] = [
	...
	},
	{
		...
		subTasks: [
			...
			{
				id: "s-22",
				description: "Publish interaction mockups",
				isCompleted: false,
			},
		],
	},
	{
		...
		subTasks: [
			{
				id: "s-31",
				description: "Create the set of icons",
				isCompleted: false,
			},
	...
	},
	{
		...
		subTasks: [
			...
			{
				id: "s-42",
				description: "Synthesize feedback themes",
				isCompleted: false,
			},
		],
	},
	{
		...
		subTasks: [
			{
				id: "s-51",
				description: "Build reusable timeline components",
				isCompleted: false,
			},
			{
				id: "s-52",
				description: "Connect chart interactions",
				isCompleted: false,
			},
		],
	},
	{
		...
		subTasks: [
			{
				id: "s-61",
				description: "Hand-off component specs",
				isCompleted: false,
			},
		],
	},
...
export default function Gantt() {
	...
	const employeeIndex = useMemo(() => {
		return new Map(
			visibleEmployees.map((employee, index) => [employee.id, index]),
		);
	}, [visibleEmployees]);
	...
	const chartWidth = useMemo(
		() => Math.max(940, (timelineRange.dayCount + 1) * DAY_COLUMN_WIDTH),
		[timelineRange.dayCount],
	);
	...
	const modalEmployee = selectedEmployeeId
		...
		return {
			...
			tooltip: {
				...
				formatter: function formatter(
					...
					const projectName =
						point.options?.custom?.projectName ?? point.name ?? "Untitled";
					const firstSubTask =
						point.options?.custom?.firstSubTask ?? "No subtasks yet";
				...
				positioner: function positioner(
					...
					const x =
						...
						labelWidth / 2;
					const y =
						(shape.y ?? 0) + this.chart.plotTop + (shape.height ?? 0) + 12;
					return { x, y };
				},
			},
			plotOptions: {
					...
					data: visibleTasks.map((task) => {
						...
						return {
							...
							custom: {
								...
								firstSubTask:
									task.subTasks[0]?.description ?? "No subtasks yet",
								color: barColor,
							},
						};
							...
							drop: function drop(
								...
								const nextRowIndex = hasYChange
									? clamp(
											Math.round(draggedY),
											0,
											Math.max(visibleEmployees.length - 1, 0),
										)
									: null;
								const nextEmployeeId =
									nextRowIndex === null
										? null
										: (visibleEmployees[nextRowIndex]?.id ?? null);

								const taskId =
									pointId ??
									String(
										(this.options as { custom?: { taskId?: string } }).custom
											?.taskId ?? "",
									);
								if (!taskId) return;
										...
										task.id !== taskId
											...
											: (() => {
													const updatedEmployeeId =
														nextEmployeeId ?? task.employeeId;
													const nextEndMs =
														draggedEnd === null
															? task.endDate.getTime()
															: Math.max(
																	task.startDate.getTime() + HOUR_MS,
																	snapToNearestDay(draggedEnd),
																);
													const endChanged =
														Math.abs(task.endDate.getTime() - nextEndMs) >= 1;
													const employeeChanged =
														updatedEmployeeId !== task.employeeId;
	...
	return (
		...
		>
			...
			>
				<div
					className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-4 border-b"
				...
				>
					...
					>
						...
						<img
							src={avatarDataUrl("PT", "#8FBB93", "#4A6B4E")}
							alt="Patrick avatar"
							...
							<div>
								{visibleEmployees.length === 0 ? (
									...
									visibleEmployees.map((employee, index) => {
										const active =
											selectedEmployeeId === employee.id && modalMounted;
										return (
											<button
												key={employee.id}
												type="button"
								...
								<div className="relative" style={{ width: chartWidth }}>
									...
									{dragBadge ? (
										...
										>
											...
											<span
												className="text-[10px] leading-none uppercase tracking-[0.06em]"
												style={{ color: THEME.timelineTextMuted }}
											>
												hrs
											</span>
						...
						<div className="mx-auto max-w-6xl space-y-5">
							...
							>
								<div
									...
									style={{ background: rgba(THEME.primarySoft, 0.22) }}
								/>
								<div className="relative flex flex-col items-center text-center gap-2">
									<img
										src={modalEmployee.avatarUrl}
										alt={modalEmployee.name}
										className="w-24 h-24 rounded-full border-[3px]"
										style={{
											borderColor: THEME.surface,
											boxShadow: "0 12px 26px rgba(42,58,44,0.24)",
										}}
									/>
									<h2
										className="text-[30px] font-semibold leading-tight"
										style={{ color: THEME.foreground, fontFamily: CHART_FONT }}
									>
										{modalEmployee.name}
									</h2>
							...
							>
										...
										>
											{modalProjectOptions.map((option) => (
												<option key={option} value={option}>
													{option === "all"
														? "All tasks"
														: (projectById.get(option)?.name ?? option)}
												</option>
											))}
										</select>
										<span className="text-xs" style={{ color: THEME.subtle }}>
											{filteredModalTasks.length} task
											{filteredModalTasks.length === 1 ? "" : "s"}
										</span>
									</div>
											...
											style={{
												background:
													taskView === "grid"
														...
														: "transparent",
												color:
													taskView === "grid" ? THEME.foreground : THEME.subtle,
											}}
										>
											Grid
										</button>
											...
											style={{
												background:
													taskView === "list"
														...
														: "transparent",
												color:
													taskView === "list" ? THEME.foreground : THEME.subtle,
											}}
										>
											List
										</button>
								...
								{filteredModalTasks.length === 0 ? (
									<div
										className="mt-5 rounded-xl border px-4 py-8 text-center text-sm"
																...
																<span
																	style={{
																		textDecoration: subTask.isCompleted
																			? "line-through"
																			: "none",
																		opacity: subTask.isCompleted ? 0.75 : 1,
																	}}
																>
																	{subTask.description}
																</span>
			...
			{closingMask ? (
				<div
					...
					style={{
						...
						animation:
							"gantt-close-circle 560ms cubic-bezier(0.22, 0.84, 0.26, 1) forwards",
					}}
				/>
			) : null}
