	...
	return (
		...
		>
			...
			{sub && (
				<div
					className="text-xs"
					style={{ color: highlight ? "#6A9A6E" : "#9AAE9C" }}
				>
					{sub}
				</div>
			)}
...
function TagFilterBar({
	...
	return (
		<div
			...
			style={{ borderColor: "#DDD0BC", background: "#FFFFFF" }}
		>
			<div
				className="px-4 py-3"
				style={{
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<p className="text-xs mt-1" style={{ color: "#6E8472" }}>
							{matchedCount} of {totalCount} prompts matched · {trackedCount}{" "}
							tracked
						</p>
					</div>
...
function MentionRateChart({ data }: { data: CompetitorSeries[] }) {
	...
	const options: Highcharts.Options = {
		...
		yAxis: {
			...
			labels: {
				format: "{value}%",
				style: { color: "#9AAE9C", fontSize: "11px" },
			},
			title: { text: null },
		},
		plotOptions: {
			bar: {
				...
				dataLabels: {
					...
					formatter: function () {
						const isHC = (this.point as { isHighcharts?: boolean })
							.isHighcharts;
...
export default function Competitors() {
	...
	const gapToLeader =
		hc && leader && !leader.isHighcharts
			? (leader.mentionRatePct - hc.mentionRatePct).toFixed(1)
			: null;

	const filteredTrackedCount = filteredPrompts.filter(
		(prompt) => prompt.status === "tracked",
	).length;
				...
				{isLoading ? (
					...
					<>
						...
						<StatCard
							...
							sub={
								gapToLeader
									? `${gapToLeader}% behind ${leader?.entity}`
									: "leading the field"
							}
							highlight
						/>
										...
										return (
											<tr
												...
												onMouseLeave={(e) => {
													(
														e.currentTarget as HTMLTableRowElement
													).style.background = isHC ? "#EEF3EE" : "transparent";
												}}
											>
												{/* Rank */}
												<td
