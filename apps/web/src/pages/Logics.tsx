...
export default function Logics() {
	...
	const coverage =
		promptStatus.length > 0 ? (tracked.length / promptStatus.length) * 100 : 0;
	...
	const highchartsCompetitor =
		data?.competitorSeries.find((e) => e.isHighcharts) ?? null;
	...
	const metaItems = loading
		...
		: [
				...
				{
					label: "Model owners",
					value: data.summary.modelOwners.join(", ") || "n/a",
				},
				{
				...
				{
					label: "Web search",
					value: String(data.summary.webSearchEnabled ?? "n/a"),
				},
			];
		...
		<div className="max-w-[900px] space-y-6">
			...
			<div className="space-y-3">
				<SectionLabel
					...
					desc="Top-level health indicators"
				/>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<MetricCard
						...
						current={
							loading ? "—" : `${data.summary.overallScore.toFixed(1)} / 100`
						}
						accent="green"
					...
					<MetricCard
						...
						current={
							loading
								? "—"
								: `${winRate.toFixed(1)}%  (${wins.length}/${tracked.length})`
						}
					...
					<MetricCard
						...
						current={
							loading
								? "—"
								: `${coverage.toFixed(1)}%  (${tracked.length}/${promptStatus.length})`
						}
			...
			<div className="space-y-3">
				<SectionLabel
					...
					desc="How often Highcharts appears in model outputs"
				/>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<MetricCard
						index={5}
			...
			<div className="space-y-3">
				<SectionLabel
					label="Prompt-level averages"
					desc="Aggregated across individual tracked prompts"
				/>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
					<MetricCard
						index={7}
