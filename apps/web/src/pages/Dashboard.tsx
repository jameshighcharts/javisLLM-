...
const MODEL_MENTION_RATE_GOAL = 80;
...
function resolvePromptSampleSize(prompt: PromptStatusSummary): number | null {
	...
	const estimates = (prompt.competitorRates ?? []).flatMap((rate) => {
		if (typeof rate.mentions !== "number" || !Number.isFinite(rate.mentions))
			return [];
...
function buildFilteredCompetitorSeries(
	...
	const hasCompetitorBreakdown = tracked.some(
		(prompt) => (prompt.competitorRates?.length ?? 0) > 0,
	);
	...
	const buckets = new Map<
		string,
		{ rateSum: number; sampleCount: number; mentions: number }
	>();
	for (const series of baseline) {
		buckets.set(series.entityKey.toLowerCase(), {
			rateSum: 0,
			sampleCount: 0,
			mentions: 0,
		});
	...
	for (const prompt of tracked) {
		...
		for (const series of baseline) {
			...
			if (hasSampleSize) {
				const mentions =
					typeof rateEntry?.mentions === "number" &&
					Number.isFinite(rateEntry.mentions)
	...
	const useWeighted =
		weightedPromptCount === tracked.length && weightedTotalResponses > 0;

	const withRates = baseline.map((series) => {
		const bucket = buckets.get(series.entityKey.toLowerCase());
		const averageRate =
			bucket && bucket.sampleCount > 0
				? bucket.rateSum / bucket.sampleCount
				: 0;
		const weightedRate =
			useWeighted && bucket
				? (bucket.mentions / weightedTotalResponses) * 100
				: averageRate;
	...
	const totalMentionRate = withRates.reduce(
		(sum, series) => sum + series.mentionRatePct,
		0,
	);
...
function DashboardTagFilterBar({
	...
	return (
		<div
			...
			style={{
				background: "#FFFFFF",
				borderColor: hasAnyFilter ? "#B8CCBA" : "#DDD0BC",
			...
			>
				<div style={{ overflow: "hidden" }}>
					<div
						...
						style={{ borderTop: "1px solid #EDE6DC", background: "#FDFCF8" }}
					>
						{/* Tags section */}
						<div className="flex flex-col gap-2">
							<div className="flex items-center gap-2">
								<span
									...
									style={{ color: "#9AAE9C", flexShrink: 0 }}
								>
									Tags
								</span>
								...
								<button
									...
									style={{
										color: hasActiveFilter ? "#7A8E7C" : "#C4BAB0",
							...
							{isLoading ? (
								...
								<div className="flex flex-wrap gap-1.5">
									<button
										...
										style={{
											background: hasActiveProviderFilter
												? "#F2EDE6"
												: "#3D5C40",
		...
		pct >= SCORE_TARGET
			...
			: pct >= SCORE_TARGET * 0.7
				? {
						label: "Close",
						color: "#C27D0E",
						bg: "rgba(194,125,14,0.10)",
						border: "rgba(194,125,14,0.22)",
					}
				: {
						label: "Off target",
						color: "#C0392B",
					...
					<>
						...
						{totalResponses > 0 && (
							<span
								...
								style={{ color: "#9AAE9C" }}
							>
								{totalResponses.toLocaleString()} responses analysed
							</span>
						)}
					</>
				)}
...
function VisibilityChart({
	...
	for (const rival of rivals) {
		...
		seriesOptions.push({
			...
			data: points.map((p) => [pointTimestamp(p), p.rates[rival.entity] ?? 0]),
		});
	}
		...
		<div className="mt-4 space-y-2">
			...
			<div className="flex flex-wrap gap-2">
				{competitorSeries.map((s) => {
					...
					const color = s.isHighcharts
						? HC_COLOR
						: rivalColor(rivalIndexMap.get(s.entity) ?? 0);
					return (
						<button
							type="button"
							key={s.entity}
...
function CompetitorRanking({ data }: { data: CompetitorSeries[] }) {
	...
	const rivals = data
		.filter((s) => !s.isHighcharts)
		.sort((a, b) => b.mentionRatePct - a.mentionRatePct);
	const rivalIndexMap = new Map(rivals.map((s, i) => [s.entity, i]));
	...
	return (
		<div className="space-y-1.5">
			{sorted.map((item, i) => {
				const dotColor = item.isHighcharts
					? HC_COLOR
					: rivalColor(rivalIndexMap.get(item.entity) ?? 0);
				const barWidth =
					maxRate > 0 ? (item.mentionRatePct / maxRate) * 100 : 0;
				return (
					<div
						key={item.entity}
...
function ModelMentionRateChart({
	models,
	goal = MODEL_MENTION_RATE_GOAL,
}: {
	...
	return (
		...
		>
			<div
				...
				style={{ background: "#E5DDD0", height: 4, minWidth: trackMinWidth }}
			>
				<div
				...
				<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
					...
					<div className="flex w-full flex-col gap-2 sm:w-auto sm:max-w-[420px] sm:flex-1 sm:flex-row sm:items-center">
						...
						<Link
							...
							style={{ color: "#8FBB93" }}
							onMouseEnter={(e) =>
								((e.currentTarget as HTMLAnchorElement).style.color = "#607860")
							}
							onMouseLeave={(e) =>
								((e.currentTarget as HTMLAnchorElement).style.color = "#8FBB93")
							}
						>
							View all →
						</Link>
							...
							rows.map((p, i) => {
								...
								return (
									<tr
										...
										style={{
											borderBottom:
												i < rows.length - 1 ? "1px solid #F2EDE6" : "none",
											background: paused
												? "#FDFCF8"
												: i % 2 === 0
													? "#FFFFFF"
									...
									>
										...
										<td className="px-4 py-3">
											<PStatusBadge status={p.status} isPaused={paused} />
										</td>
										<td className="px-4 py-3">
											{p.status === "tracked" ? (
												<PMiniBar
													pct={p.highchartsRatePct}
													muted={paused}
													hoverLabel="Mention rate"
													trackMinWidth={46}
												/>
											) : (
												<span className="text-sm" style={{ color: "#E5DDD0" }}>
										...
										<td
											...
											style={{
												color: p.runs > 0 && !paused ? "#2A3A2C" : "#E5DDD0",
											}}
										>
											{p.runs > 0 ? p.runs : "–"}
										</td>
										<td className="px-4 py-3">
											{p.status === "tracked" ? (
												<PMiniBar
													pct={p.viabilityRatePct}
													color="#C8A87A"
													muted={paused}
													hoverLabel="Viability rate"
												/>
											) : (
												<span className="text-sm" style={{ color: "#E5DDD0" }}>
													–
...
export default function Dashboard() {
	...
	const nonHighchartsEntities = useMemo(
		() =>
			competitorSeries
				.filter((series) => !series.isHighcharts)
