...
function StatusBadge({
	...
	return (
		...
		>
			<span
				className="w-1.5 h-1.5 rounded-full"
				style={{ background: tracked ? "#22c55e" : "#C8DEC9" }}
			/>
...
export default function PromptDrilldownHub() {
	...
	const tracked = prompts.filter(
		(prompt) => prompt.status === "tracked" && !prompt.isPaused,
	);
	const paused = prompts.filter((prompt) => prompt.isPaused);

	const avgHighcharts =
		tracked.length > 0
			? tracked.reduce((sum, prompt) => sum + prompt.highchartsRatePct, 0) /
				tracked.length
			: 0;
	...
	const avgLead =
		tracked.length > 0
			? tracked.reduce(
					(sum, prompt) =>
						sum +
						(prompt.highchartsRatePct - (prompt.topCompetitor?.ratePct ?? 0)),
					0,
				) / tracked.length
			: 0;
	...
	const filtered = useMemo(() => {
		const rows = prompts.slice().sort((left, right) => {
		...
		<div className="max-w-[1360px] space-y-4">
			<div>
				...
				<p className="text-sm mt-0.5" style={{ color: "#7A8E7C" }}>
					Pick a prompt, then drill into output text, mentions, and per-run
					scoring.
				</p>
			</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
				...
				<SummaryCard
					...
					sub="Active with run data"
					accent="#2A5C2E"
				/>
				<SummaryCard
					label="Paused"
					value={String(paused.length)}
					sub="Excluded from next run"
				/>
				<SummaryCard
					label="Avg Lead"
						...
						<thead>
							<tr style={{ borderBottom: "1px solid #F2EDE6" }}>
								...
								>
									<HeaderLabel
										label="Lead"
										info="Highcharts rate minus top rival rate for this prompt."
										align="right"
									/>
								</th>
								<th
						...
						<tbody>
							{isLoading
								...
								: filtered.map((prompt, index) => {
										const lead =
											prompt.highchartsRatePct -
											(prompt.topCompetitor?.ratePct ?? 0);
										return (
											<tr
												key={prompt.query}
												style={{
											...
											>
												<td className="px-4 py-3">
													<Link
														...
														style={{
															color: prompt.isPaused ? "#9AAE9C" : "#2A3A2C",
														}}
													>
												...
												<td className="px-4 py-3">
													<PromptTagChips
														tags={prompt.tags}
														muted={prompt.isPaused}
													/>
												</td>
												<td className="px-4 py-3">
													<StatusBadge
														isPaused={prompt.isPaused}
														tracked={prompt.status === "tracked"}
													/>
												</td>
												<td
												...
												<td
													className="px-4 py-3 text-right text-sm font-semibold tabular-nums"
													style={{ color: lead >= 0 ? "#2A5C2E" : "#B45309" }}
												>
													{lead >= 0 ? "+" : ""}
													{lead.toFixed(1)}%
												</td>
