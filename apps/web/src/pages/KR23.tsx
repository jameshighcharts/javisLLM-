...
const INITIATIVE_STATUS_META: Record<
	InitiativeStatus,
...
function MetricCard({
	...
	const glow =
		tone === "positive"
			? "rgba(143,187,147,0.18)"
			: tone === "warning"
				? "rgba(212,152,128,0.18)"
				: "rgba(74,107,78,0.08)";
			...
			>
				...
				<div style={{ width: 56, flexShrink: 0 }}>
					<div
						...
						style={{ height: 4, background: "#E8E1D4" }}
					>
						<div
			...
			>
				<div style={{ overflow: "hidden" }}>
					<div
						...
						style={{ borderTop: "1px solid #EDE5D6" }}
					>
						<div className="pt-2 grid gap-2 sm:grid-cols-2">
							...
							<label className="block space-y-1">
								...
								<input
									...
									max={80}
									step={0.1}
									value={task.expectedLiftM}
									onChange={(e) =>
										onPatch({
											expectedLiftM: clamp(Number(e.target.value) || 0, 0, 80),
										})
									}
									style={INPUT_STYLE}
								/>
...
export default function KR23() {
	...
	const sortedCheckpoints = useMemo(
		() =>
			[...checkpoints].sort(
				(left, right) => parseDate(left.date) - parseDate(right.date),
			),
		[checkpoints],
	);
	...
	const growthPct =
		baselineAudienceM > 0 ? (achievedDeltaM / baselineAudienceM) * 100 : 0;
	const targetGrowthPct =
		baselineAudienceM > 0 ? (requiredDeltaM / baselineAudienceM) * 100 : 0;
	const progressPct =
		requiredDeltaM > 0
			? clamp((achievedDeltaM / requiredDeltaM) * 100, 0, 100)
			: 100;
	...
	const expectedLiftM = tasks.reduce(
		(sum, task) => sum + task.expectedLiftM,
		0,
	);
	...
	const requiredTrajectory = useMemo(
		() =>
			timelineDates.map((time) => {
				...
				const ratio =
					(time - baselineMs) / Math.max(deadlineMs - baselineMs, DAY_MS);
		...
		() =>
			timelineDates.map((time) => {
				...
				return row.source === "actual"
					? Number(row.audienceM.toFixed(2))
					: null;
			}),
	...
	const audienceTrendOptions = useMemo<Highcharts.Options>(
		() => ({
			...
			xAxis: {
				categories: timelineDates.map(
					(date) => `${formatMonth(date)} ${new Date(date).getFullYear()}`,
				),
				lineColor: "#D8CCB8",
				tickColor: "#D8CCB8",
				labels: {
			...
			series: [
				{
					...
					data: tasks.map((task, index) => ({
						...
						color: ["#8FBB93", "#C8A87A", "#A89CB8", "#D49880", "#7AABB8"][
							index % 5
						],
					})),
					dataLabels: {
			...
			tooltip: {
				...
				pointFormatter: function pointFormatter(
					...
					const completion = Math.round(
						(pointWithMeta.completed?.amount ?? 0) * 100,
					);
	...
	function patchCheckpoint(
		checkpointId: string,
		next: Partial<AudienceCheckpoint>,
	) {
	...
	const deadlineTone: "neutral" | "warning" =
				...
				<div className="grid gap-4 lg:grid-cols-[1.45fr_0.85fr]">
					<div className="space-y-3">
						...
						<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
							<label className="block space-y-1">
								<span className="text-xs" style={{ color: "#6E8370" }}>
									Metric name
								</span>
								<input
									value={metricName}
							...
							<label className="block space-y-1">
								...
								<input
									...
									onChange={(event) =>
										setTargetAudienceM(
											clamp(Number(event.target.value) || 0, 1, 5000),
										)
									}
									style={INPUT_STYLE}
								/>
							...
							<label className="block space-y-1">
								...
								<input
									...
									value={deadline}
									onChange={(event) =>
										setDeadline(event.target.value || todayIso)
									}
									style={INPUT_STYLE}
								/>
							</label>
					...
					>
						...
						>
							...
							<div className="flex items-center justify-between">
								...
								<span className="tabular-nums">
									{Math.max(gapToTargetM, 0).toFixed(1)}M
								</span>
							</div>
							<div
								className="h-2 rounded-full overflow-hidden"
			...
			<section className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
				...
				>
					<div className="mb-3">
						...
						<p className="text-xs" style={{ color: "#6E8370" }}>
							Actual and forecast checkpoints against the required line to reach
							302M.
						</p>
					</div>
					<HighchartsReact
						highcharts={Highcharts}
						options={audienceTrendOptions}
					/>
				</article>
				...
				>
					...
					<div className="mt-2 text-xs" style={{ color: "#6E8370" }}>
						Combined expected uplift from plan:{" "}
						<b>{expectedLiftM.toFixed(1)}M</b>
					</div>
				</article>
			</section>
					...
					>
						<div className="mb-3 flex items-end justify-between gap-3">
							<div>
								...
								<p className="text-xs" style={{ color: "#6E8370" }}>
									Workstreams driving monthly audience growth through the May 31
									deadline.
								</p>
							</div>
							<Pill
								text={`${tasks.length} initiatives`}
								bg="#F2EDE6"
								color="#607860"
							/>
						</div>
						...
						<div className="mb-3 flex items-center justify-between gap-2">
							<div>
								...
								<p className="text-xs mt-0.5" style={{ color: "#6E8370" }}>
									Update status and progress per initiative.
								</p>
							</div>
							<Pill
						...
						{tasks.length === 0 ? (
							...
							>
								...
								<p className="mt-1 text-xs">
									Click Add to create your first initiative.
								</p>
							</div>
						) : (
			...
			>
				<div className="mb-3 flex items-center justify-between gap-2">
					<div>
						...
						<p className="text-xs" style={{ color: "#6E8370" }}>
							Track observed and forecast readings for the `{metricName}`
							metric.
						</p>
					</div>
					<Pill
						text={`${initiativeProgress.toFixed(0)}% avg initiative progress`}
						bg="#E8F6EA"
						color="#25693A"
					/>
				</div>
