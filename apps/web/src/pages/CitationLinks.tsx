...
function isHighchartsHost(host: string) {
...
function ProviderCoverageChart({
	...
	const providerCounts = useMemo(() => {
		...
		for (const src of sources) {
			const explicitCounts = src.providerCitationCounts ?? {};
			if (Object.keys(explicitCounts).length > 0) {
				for (const [provider, count] of Object.entries(explicitCounts)) {
					counts[provider] = (counts[provider] ?? 0) + count;
				}
				continue;
			}
			if (src.providers.length === 1) {
				const provider = src.providers[0];
				counts[provider] = (counts[provider] ?? 0) + src.citationCount;
			}
		}
		return counts;
	}, [sources]);
	...
	const normalizedResponsesWithCitations = Math.min(
		Math.max(0, responsesWithCitations),
		Math.max(0, totalResponses),
	);
	const responsesWithoutCitations = Math.max(
		0,
		totalResponses - normalizedResponsesWithCitations,
	);
	const citePct =
		totalResponses > 0
			...
			series: [
				{
					...
					data: [
						{
							name: "With citations",
							y: normalizedResponsesWithCitations,
							color: "#4A8C50",
						},
						{ name: "Without", y: responsesWithoutCitations, color: "#E8E4DC" },
					],
				},
			],
		}),
		[normalizedResponsesWithCitations, responsesWithoutCitations],
	);
...
export default function CitationLinks() {
	...
	const filteredSources = useMemo((): CitationLinksSourceStat[] => {
		...
		if (domainFilter === "internal")
			return sources.filter((s) => isHighchartsHost(s.host));
				...
				<>
					...
					>
						...
						>
							...
							<div className="text-xs mt-0.5" style={{ color: "#9AAE9C" }}>
								Highcharts.com properties vs third-party domains
							</div>
						</div>
						<div
							...
							<div className="p-4">
								...
								{citationDomainSplit.internal.length === 0 && (
									<p className="text-xs" style={{ color: "#B5C4B7" }}>
										No internal citations in this run
									</p>
								)}
							</div>
							<div className="p-4">
								...
								{citationDomainSplit.external.length === 0 && (
									<p className="text-xs" style={{ color: "#B5C4B7" }}>
										No external citations in this run
									</p>
								)}
							</div>
						</div>
					...
					{filteredSources.length > 0 && (
						<>
							...
							<CitationSourceLeaderboard
								...
								subtitle={`${data.runMonth ?? "Latest run"} · ${filteredSources.length.toLocaleString()} unique domains · ${filteredSources.reduce((sum, source) => sum + source.citationCount, 0).toLocaleString()} citations`}
								limit={limit}
							/>
						</>
					)}
