...
const initDraggablePoints = DraggablePoints as unknown as (
	chartingLibrary: typeof Highcharts,
) => void;
...
function getDraggedEnd(
	...
	if (pointId && event.newPoints?.[pointId])
		candidates.push(event.newPoints[pointId]);
...
function getPointId(point: Highcharts.Point): string | null {
	...
	return (
		candidate.id ??
		candidate.options?.id ??
		candidate.options?.custom?.taskId ??
		null
	);
}
...
function getPointEndMs(point: Highcharts.Point): number {
	...
	return (
		candidate.end ??
		candidate.x2 ??
		candidate.options?.end ??
		candidate.options?.x2 ??
		Date.now()
	);
}
...
function sortTimelineDraftTasks(
	a: TimelineDraftTask,
	b: TimelineDraftTask,
): number {
...
function inferDragMode(
	...
	return Math.abs(nextStart - originalStartMs) >=
		Math.abs(nextEnd - originalEndMs)
		? "resize-start"
		: "resize-end";
}
...
function resolveSingleTrackDrag(input: {
	...
	if (mode === "resize-start") {
		...
		if (!linkedPushApplied) {
			const { minStart } = getTimelineBounds(
				drafts,
				targetOwner,
				taskId,
				startMs,
		...
		if (!linkedPushApplied) {
			const { maxEnd } = getTimelineBounds(
				drafts,
				targetOwner,
				taskId,
				startMs,
		...
		let attempts = 0;
		while (attempts < 4) {
			const { minStart, maxEnd } = getTimelineBounds(
				drafts,
				active.owner,
				taskId,
				startMs,
...
function normalizeTimelineByOwner(
	drafts: TimelineDraftTask[],
): TimelineDraftTask[] {
	...
	for (const row of rows.values()) {
		...
		for (let index = 0; index < row.length; index += 1) {
			...
			if (current.endMs <= current.startMs)
				current.endMs = current.startMs + WEEK_MS;
			if (current.endMs - current.startMs < WEEK_MS)
				current.endMs = current.startMs + WEEK_MS;
			if (index > 0) {
...
const INITIAL_PAGES: ComparisonPage[] = [
	{
		id: "page-1",
		title: "AG Charts vs Highcharts",
		status: "launched",
		qualityScore: 88,
		launchedOn: "2026-02-18",
	},
	{
		id: "page-2",
		title: "Recharts vs Highcharts",
		status: "launched",
		qualityScore: 84,
		launchedOn: "2026-02-26",
	},
	{
		id: "page-3",
		...
		status: "launched",
		qualityScore: 81,
		launchedOn: "2026-03-04",
	},
	{
		id: "page-4",
		title: "Chart.js vs Highcharts",
		status: "qa",
		qualityScore: 79,
		launchedOn: null,
	},
	...
	{
		id: "page-6",
		title: "ApexCharts vs Highcharts",
		status: "draft",
		qualityScore: 74,
		launchedOn: null,
	},
	...
	{
		id: "page-10",
		title: "Observable Plot vs Highcharts",
		status: "ideation",
		qualityScore: 65,
		launchedOn: null,
	},
...
function MetricCard({
	...
	const dot = { positive: "#4A6B4E", warning: "#C06040", neutral: "#A8B0A8" }[
		tone
	];
	const badgeBg = {
		positive: "#E8F5EA",
...
function WorkstreamCard({
	...
	return (
		...
		>
			...
			{expanded && (
				<div
					className="p-3 pt-3 space-y-2"
					style={{ borderTop: "1px solid #E0D5C4", marginTop: 6 }}
				>
					...
					<div className="grid gap-2 sm:grid-cols-2">
						...
						<label className="block space-y-1">
							...
							<input
								...
								onChange={(e) =>
									onPatch({
										pagesTarget: clamp(Number(e.target.value) || 0, 0, 20),
									})
								}
								style={INPUT_STYLE}
							/>
					...
					<label className="block space-y-1">
						...
						<select
							...
							onChange={(e) =>
								onPatch({ dependency: e.target.value || undefined })
							}
							style={{ ...INPUT_STYLE }}
						>
					...
					<div>
						<div className="flex items-center justify-between mb-1">
							...
							<input
								...
								onChange={(e) =>
									onPatch({
										progress: clamp(Number(e.target.value) || 0, 0, 100),
									})
								}
								style={{
									...INPUT_STYLE,
...
export default function OKR() {
	...
	const [checklistName, setChecklistName] = useState(
		"LLM Comparison Quality Checklist v1",
	);
	...
	const [resizeBadge, setResizeBadge] = useState<{
		left: number;
		top: number;
		hours: number;
	} | null>(null);
	...
	const taskColorMap = useMemo(
		() => buildTaskColorMap(tasks.map((task) => task.id)),
		[tasks],
	);

	// ── Metrics ────────────────────────────────────────────────────────────────
	const launchedPages = useMemo(
		() => pages.filter((p) => p.status === "launched"),
		[pages],
	);
	const launchedCount = launchedPages.length;
	const remainingPages = Math.max(targetPages - launchedCount, 0);
	const launchProgressPct =
		targetPages > 0 ? clamp((launchedCount / targetPages) * 100, 0, 100) : 0;
	const avgLaunchedQuality = average(launchedPages.map((p) => p.qualityScore));
	const qualityPassCount = launchedPages.filter(
		(p) => p.qualityScore >= qualityThreshold,
	).length;
	const qualityPassRate =
		launchedCount > 0 ? (qualityPassCount / launchedCount) * 100 : 0;
	const taskProgressPct = average(tasks.map((t) => t.progress));
	const blendedProgressPct = clamp(
		launchProgressPct * 0.62 + qualityPassRate * 0.2 + taskProgressPct * 0.18,
		0,
		100,
	);
	const earliestTaskStartMs =
		tasks.length > 0
			? Math.min(...tasks.map((t) => parseDate(t.start)))
			: todayMs;
	...
	const requiredVelocity =
		daysRemaining <= 0 && remainingPages > 0
			...
			: weeksRemaining > 0
				...
				: 0;
	const qualityReady =
		launchedCount === 0 ? false : avgLaunchedQuality >= qualityThreshold;
	const onTrack =
		remainingPages === 0
	...
	const timelineRange = useMemo(() => {
		const allTimes = visibleTasks.flatMap((task) => [
	...
	const executionTimelineHeight = useMemo(
		() =>
			OKR_GANTT_HEADER_HEIGHT +
			Math.max(1, ownerRows.length) * OKR_GANTT_ROW_HEIGHT,
		[ownerRows.length],
	);
	...
	useEffect(() => {
		if (
			hoveredTaskId &&
			!visibleTasks.some((task) => task.id === hoveredTaskId)
		) {
	...
	const projectedCumulative = useMemo(() => {
		...
		return checkpointDates.map((cp) => {
			...
			return Number(
				Math.min(targetPages, currentVelocity * weeksFromStart).toFixed(2),
			);
		});
	...
	const ganttOptions = useMemo<Highcharts.Options>(() => {
		return {
			...
			xAxis: {
				...
				currentDateIndicator: {
					...
					dashStyle: "ShortDot",
					label: {
						style: { color: OKR_GANTT_THEME.timelineAxisSub, fontSize: "11px" },
					},
				},
				plotLines: [
					{
						...
						label: {
							...
							y: 12,
							style: {
								color: OKR_GANTT_THEME.timelineAxisSub,
								fontSize: "11px",
								fontWeight: "600",
							},
						},
				...
				labels: {
					...
					y: 18,
					style: {
						color: OKR_GANTT_THEME.timelineAxis,
						fontSize: "11px",
						fontWeight: "600",
						zIndex: "7",
					},
			...
			tooltip: {
				...
				shadow: false,
				padding: 10,
				formatter: function pointFormatter(
			...
			series: [
				{
					...
					dataLabels: {
						...
						formatter: function formatter(
							...
							const textColor =
								p.options?.custom?.textColor ?? OKR_GANTT_THEME.timelineText;
					...
					point: {
						events: {
							mouseOver: function mouseOver(this: Highcharts.Point): void {
								...
								const graphic = (
									this as Highcharts.Point & {
										graphic?: { toFront?: () => void };
									}
								).graphic;
								graphic?.toFront?.();
							},
							...
							dragStart: function dragStart(this: Highcharts.Point): void {
								...
								const rowTasks = drafts
									.filter(
										(task) =>
											task.owner === active.owner && task.id !== active.id,
									)
									.sort(sortTimelineDraftTasks);

								const linkedLeftTaskId =
									rowTasks
										.filter(
											(task) => Math.abs(task.endMs - active.startMs) < HOUR_MS,
										)
										.sort((a, b) => b.endMs - a.endMs)[0]?.id ?? null;

								const linkedRightTaskId =
									rowTasks
										.filter(
											(task) => Math.abs(task.startMs - active.endMs) < HOUR_MS,
										)
							...
							drag: function drag(
								...
								if (draftEndMs <= draftStartMs) {
									if (mode === "resize-start")
										draftStartMs = draftEndMs - WEEK_MS;
								...
								const targetOwner =
									nextRowIndex === null
										? normalizeOwner(
												tasksRef.current.find((task) => task.id === pointId)
													?.owner ?? session.originalOwner,
											)
								...
								const resolved = resolveSingleTrackDrag({
									...
									allowLinkedPush:
										targetOwner === session.originalOwner && mode !== "move",
								});
								...
								for (const [id, draft] of resolved.entries()) {
									const point =
										id === pointId
											? this
											: this.series.points.find(
													(candidate) => getPointId(candidate) === id,
												);
									if (!point) continue;
								...
								const hours = Math.max(
									1,
									Math.round(
										(activeDraft.endMs - activeDraft.startMs) / HOUR_MS,
									),
								);
								const chart = this.series.chart;
								const shape = (
									this as Highcharts.Point & { shapeArgs?: { y?: number } }
								).shapeArgs;
								const anchorMs =
									mode === "resize-start"
										? activeDraft.startMs
							...
							drop: function drop(
								...
								if (snappedEndMs <= snappedStartMs) {
									if (mode === "resize-start")
										snappedStartMs = snappedEndMs - WEEK_MS;
								...
								setTasks((currentTasks) => {
									...
									const resolved = resolveSingleTrackDrag({
										...
										allowLinkedPush:
											!!session &&
											targetOwner === session.originalOwner &&
											mode !== "move",
									});
									...
									const normalized = new Map(
										normalizeTimelineByOwner(merged).map((task) => [
											task.id,
											task,
										]),
									);
									...
									return currentTasks.map((task) => {
										...
										const ownerChanged =
											next.owner !== normalizeOwner(task.owner);
		...
		deadlineMs,
		executionTimelineHeight,
		executionTimelineWidth,
		hoveredTaskId,
		isDragging,
		ownerIndex,
		ownerRows,
	...
	function patchPage(pageId: string, next: Partial<ComparisonPage>) {
		setPages((cur) =>
			cur.map((p) => (p.id === pageId ? { ...p, ...next } : p)),
		);
	}

	function patchTask(taskId: string, next: Partial<WorkstreamTask>) {
		setTasks((cur) =>
			cur.map((t) => (t.id === taskId ? { ...t, ...next } : t)),
		);
	}
	...
	function removeTask(taskId: string) {
		setTasks((cur) =>
			cur
				...
				.map((t) =>
					t.dependency === taskId ? { ...t, dependency: undefined } : t,
				),
		);
	...
	const daysTone: "neutral" | "warning" =
		daysRemaining > 14 ? "neutral" : "warning";
		...
		<div className="space-y-4 pb-4">
			...
			<section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
				<MetricCard
					...
					bar={{
						pct: avgLaunchedQuality,
						color:
							avgLaunchedQuality >= qualityThreshold ? "#4A6B4E" : "#C8A87A",
					}}
				/>
				<MetricCard
					label="Status"
					...
					sub={
						onTrack
							? "Launch pace and quality are above threshold."
							: "Velocity or quality pass rate is below target."
					}
					tone={statusTone}
				...
				<MetricCard
					...
					bar={{
						pct:
							Number.isFinite(requiredVelocity) && requiredVelocity > 0
								? clamp((currentVelocity / requiredVelocity) * 100, 0, 100)
								: 100,
						color: statusTone === "positive" ? "#4A6B4E" : "#C06040",
					}}
				/>
				<MetricCard
					...
					tone={daysTone}
					bar={{
						pct: clamp(100 - (daysRemaining / 97) * 100, 0, 100),
						color: daysTone === "warning" ? "#C06040" : "#8A9C8C",
					}}
				/>
			</section>
					...
					<div className="mb-3 flex items-start justify-between gap-2">
						<div>
							...
							<p className="text-xs" style={{ color: "#6E8370" }}>
								Click to expand and edit. Every change updates the timeline
								instantly.
							</p>
						</div>
						<button
					...
					{tasks.length === 0 ? (
						...
						>
							...
							<p className="mt-1 text-xs">
								Click Add to create your first workstream.
							</p>
						</div>
					) : (
					...
					<div className="mb-3 flex items-center justify-between gap-2">
						<div>
							<h2
								...
								style={{ color: "#1F2B21" }}
							>
								Comparison Page Tracker
							</h2>
							<p className="text-xs mt-0.5" style={{ color: "#6E8370" }}>
								Update status, quality score, and launch date per page.
							</p>
						</div>
						<Pill
							text={`${launchedCount}/${targetPages} launched`}
							bg="#E8F6EA"
							color="#25693A"
						/>
					</div>
								...
								>
									...
									<div className="flex items-start justify-between gap-2 mb-2.5">
										...
										<div style={{ position: "relative", flexShrink: 0 }}>
											...
											>
												{(Object.keys(STATUS_META) as ComparisonStatus[]).map(
													(s) => (
									...
									<div className="flex items-center gap-3">
										...
										{page.status === "launched" ? (
											<input
												...
												onChange={(e) =>
													patchPage(page.id, {
														launchedOn: e.target.value || null,
													})
												}
												style={{
													flexShrink: 0,
				...
				>
					<div className="mb-3">
						<h2 className="text-sm font-semibold" style={{ color: "#1F2B21" }}>
							Launch Trend vs Plan
						</h2>
						<p className="text-xs" style={{ color: "#6E8370" }}>
							Planned vs actual cumulative launches, plus projection at current
							pace.
						</p>
					</div>
