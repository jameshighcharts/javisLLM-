...
function PromptTrendChart({
	...
	const highchartsEntity =
		competitors.find((row) => row.isHighcharts)?.entity ?? null;
...
function CompetitorBreakdownChart({
	competitors,
}: {
	competitors: PromptDrilldownCompetitor[];
}) {
	...
	return (
		<div className="overflow-x-auto">
			<table className="w-full min-w-[720px] border-collapse">
				<thead>
					<tr
						style={{ borderBottom: "1px solid #F2EDE6", background: "#FDFCF8" }}
					>
						<th
								...
								{point.topCompetitor ? (
									<div className="flex items-center gap-1.5">
										{getEntityLogo(point.topCompetitor.entity) && (
											<EntityLogo
												entity={point.topCompetitor.entity}
												size={14}
											/>
										)}
										<span
...
function ResponseExplorer({
	...
	const responseSearchRows = useMemo(
		() =>
			responses.map((response) => {
				...
				const modelOwner =
					response.modelOwner ?? inferModelOwner(response.model);
				return {
					response,
					modelOwner,
					tags,
	...
	const effectiveRunId =
		selectedRunId === "all" ||
		runOptions.some((run) => run.runId === selectedRunId)
			? selectedRunId
			: "all";
	...
	const availableTags = useMemo(() => {
		return [
			...new Set(
				responseSearchRows
					.flatMap((row) => Object.keys(row.tags))
					.filter(Boolean),
			),
	...
	const availableOwners = useMemo(() => {
		return [
			...new Set(
				responseSearchRows.map((row) => row.modelOwner).filter(Boolean),
			),
		].sort((left, right) => left.localeCompare(right));
	}, [responseSearchRows]);
	...
	const effectiveOwner =
		selectedOwner === "all" || availableOwners.includes(selectedOwner)
			? selectedOwner
			: "all";
	...
	const filteredResponses = useMemo(() => {
		...
		return responseSearchRows
			.filter(
				(row) =>
					effectiveRunId === "all" || row.response.runId === effectiveRunId,
			)
			.filter(
				(row) => effectiveOwner === "all" || row.modelOwner === effectiveOwner,
			)
			.filter((row) => {
				const tagValue =
					effectiveTag === "all" ? "" : (row.tags[effectiveTag] ?? "");

				if (effectiveTag !== "all" && !tagValue) return false;
				if (!term) return true;

				const haystack =
					effectiveTag === "all" ? row.allText : tagValue.toLowerCase();
	...
	if (responses.length === 0) {
		return (
			<div className="text-sm text-center py-10" style={{ color: "#9AAE9C" }}>
				No prompt outputs yet. Run benchmark to generate output text and mention
				matches.
			</div>
		);
	}
		...
		<div className="space-y-4">
			<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				...
				<div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:flex sm:flex-wrap sm:items-center">
					<label
						className="flex items-center justify-between gap-2 text-sm sm:text-xs sm:inline-flex sm:justify-start"
						style={{ color: "#7A8E7C" }}
					>
						Run filter
						<select
							...
							{runOptions
								...
								.map((run) => (
									<option key={run.runId} value={run.runId}>
										{run.runMonth ?? shortRunId(run.runId)} ·{" "}
					...
					>
						...
						<input
							...
							placeholder={
								effectiveTag === "all"
									? "Find text in outputs"
									: `Find text in ${effectiveTag}`
							}
			...
			<div className="space-y-3 max-h-[680px] overflow-y-auto pr-1">
				...
				{filteredResponses.map((response) => {
					...
					const promptTokens = Math.max(
						0,
						Math.round(response.promptTokens ?? 0),
					);
					const completionTokens = Math.max(
						0,
						Math.round(response.completionTokens ?? 0),
					);
							...
							<div className="flex items-start justify-between gap-3 flex-wrap">
								<div className="space-y-0.5">
									<div
										...
										style={{ color: "#2A3A2C" }}
									>
										Run {response.runMonth ?? shortRunId(response.runId)} · #
										{response.runIteration}
									</div>
									<div className="text-xs" style={{ color: "#9AAE9C" }}>
										{formatDateTime(
											response.createdAt ?? response.runCreatedAt,
										)}
									</div>
								</div>
								...
								<div className="flex items-center gap-2 text-xs">
									...
									<span
										className="px-2 py-0.5 rounded-full font-medium"
										style={{
											background: response.webSearchEnabled
												? "#ECFDF3"
												: "#F8FAFC",
									...
									<span
										...
										title={
											pricing
												? `${formatUsdPerMillion(pricing.inputUsdPerMillion)} in, ${formatUsdPerMillion(pricing.outputUsdPerMillion)} out`
												: "No pricing found"
										}
									>
										{estimatedCosts
											? formatUsd(estimatedCosts.totalCostUsd)
											: "cost n/a"}
									</span>
								</div>
							</div>
	...
	const costSummary = useMemo(() => {
		...
		return {
			...
			avgCostPerResponseUsd:
				pricedResponseCount > 0 ? totalCostUsd / pricedResponseCount : 0,
		};
	}, [data]);
	...
	return (
		<div className="max-w-[1200px] space-y-4">
			...
			{loading ? (
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
					...
					<SummaryCard
						...
						sub={
							data.summary.topCompetitor
								? `vs ${data.summary.topCompetitor.entity}`
								: "No rival data"
						}
					...
					<SummaryCard
						...
						sub={
							data.summary.lastRunAt
								? `Last run ${formatDateTime(data.summary.lastRunAt)}`
								: "No runs yet"
						}
					/>
					<SummaryCard
									...
									<tbody>
										{costSummary.modelRows.map((row, index) => {
											const avgCostPerResponse =
												row.responseCount > 0
													? row.totalCostUsd / row.responseCount
													: 0;
											return (
												<tr
													key={row.model}
												...
												>
													...
													>
														{row.pricing
															? formatUsdPerMillion(
																	row.pricing.inputUsdPerMillion,
																)
															: "n/a"}
													</td>
													<td
													...
													>
														{row.pricing
															? formatUsdPerMillion(
																	row.pricing.outputUsdPerMillion,
																)
															: "n/a"}
													</td>
													<td
			...
			{loading ? (
				...
				>
					<div
						className="h-[220px] rounded-lg animate-pulse m-4"
						style={{ background: "#E5DDD0" }}
					/>
				</div>
			) : (
