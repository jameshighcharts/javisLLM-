...
function AnimatedNumber({
	...
	useEffect(() => {
		...
		const step = (timestamp: number) => {
			...
			const easeProgress = 1 - (1 - progress) ** 4; // easeOutQuart
	...
	return (
		...
		>
			...
			<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
				...
				<div
					className="bg-white rounded-xl border px-4 py-2 text-sm font-medium text-gray-600 shadow-sm flex items-center gap-2"
					style={{ borderColor: "rgba(221, 208, 188, 0.4)" }}
				>
					Segment by Tags & Model{" "}
					<span className="text-gray-400">| 128 prompts · all providers</span>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
				...
				>
					...
					<div className="flex justify-between text-xs text-gray-400 mt-2 font-medium">
						...
						<span className="relative">
							<span className="absolute -top-6 -left-3 text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-500">
								Goal
							</span>
							50
						</span>
				...
				<div className="grid grid-cols-2 gap-4 col-span-1">
					...
					>
						...
						<div className="text-[10px] font-bold text-[#558A5E] uppercase tracking-widest mt-1">
							Active Queries
						</div>
					</motion.div>
					...
					>
						<div className="flex justify-between items-center">
							...
							<div className="p-1.5 bg-[#EFF6F0] rounded-lg text-[#558A5E]">
								<Search size={14} />
							</div>
						</div>
						<div className="text-sm font-bold text-[#2A3A2C] mt-2 mb-4">
							Manage Queries
						</div>
						...
						<div className="text-xs font-bold text-[#558A5E] mt-2">
							Open Prompts →
						</div>
					</motion.div>
				</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
				...
				>
					<div className="flex justify-between items-start mb-6">
						<div>
							<h4 className="text-lg font-bold text-[#2A3A2C]">
								LLM Mention Rate Over Time
							</h4>
						...
						<div className="text-right">
							<div className="text-3xl font-black text-[#2A3A2C]">
								<AnimatedNumber value={61.5} decimals={1} suffix="%" />
							</div>
					...
					<div className="flex-1 min-h-[200px] relative w-full border-b border-l border-gray-100 mb-6">
						...
						>
							...
							<motion.path
								...
								transition={{ delay: 1 }}
								d="M 0 50 Q 50 30 100 40 L 100 100 L 0 100 Z"
								fill="#558A5E"
							/>

							{[
								{
									d: "M 0 5 Q 50 1 100 15",
									color: "#D6A15C",
									delay: 0.1,
									strokeDasharray: "2 2",
								}, // D3
								...
								{
									d: "M 0 30 Q 60 30 100 35",
									color: "#D18880",
									delay: 0.3,
									strokeDasharray: "2 2",
								}, // Echarts
								{
									d: "M 0 50 Q 50 30 100 40",
									color: "#558A5E",
									delay: 0.5,
									strokeWidth: 3,
								}, // Highcharts
					...
					<div className="flex flex-wrap gap-2 mt-4">
						<div className="px-3 py-1 rounded-full text-xs font-bold border border-[#558A5E] text-[#558A5E] bg-[#F2F9F4]">
							Highcharts only
						</div>
						...
						{/* Legend Pills */}
						<div className="px-3 py-1 rounded-full text-[10px] font-bold text-white bg-[#D6A15C] flex items-center gap-1.5">
							<div className="w-1.5 h-1.5 rounded-full bg-white" />
							d3.js
						</div>
						<div className="px-3 py-1 rounded-full text-[10px] font-bold text-white bg-[#558A5E] flex items-center gap-1.5">
							<div className="w-1.5 h-1.5 rounded-full bg-white" />
							Highcharts
						</div>
						<div className="px-3 py-1 rounded-full text-[10px] font-bold text-white bg-[#A894B4] flex items-center gap-1.5">
							<div className="w-1.5 h-1.5 rounded-full bg-white" />
							chart.js
						</div>
						<div className="px-3 py-1 rounded-full text-[10px] font-bold text-white bg-[#D18880] flex items-center gap-1.5">
							<div className="w-1.5 h-1.5 rounded-full bg-white" />
							echarts
						</div>
					</div>
				...
				>
					<div className="mb-4">
						<h4 className="text-[15px] font-bold text-[#2A3A2C]">
							Competitor Ranking
						</h4>
						<div className="text-[11px] text-gray-500">
							Sorted by mention rate - bar = relative share
						</div>
					</div>
					...
					<div className="flex-1 flex flex-col justify-between space-y-3 relative z-10 w-full">
						{[
							...
							<div
								key={comp.name}
								className={`flex items-center gap-3 w-full text-xs font-semibold ${comp.active ? "bg-[#F2F9F4] p-2 -mx-2 rounded-xl border border-[#CDE2D0]" : "py-1"}`}
							>
								<div className="w-4 text-center text-gray-400">{comp.rank}</div>
								<div
									className={`w-4 h-4 rounded-sm flex items-center justify-center ${comp.active ? "" : "opacity-80"}`}
								>
									{/* Mock small icon */}
									<div
										className={`w-2.5 h-2.5 rounded-[3px] bg-current opacity-80 ${comp.iconColor}`}
									></div>
								</div>
								<div
									className={`w-20 truncate ${comp.active ? "text-[#2A3A2C]" : "text-gray-600"}`}
								>
									{comp.name}
								</div>
								...
								<div className="text-right w-16">
									<div
										className={`font-bold ${comp.active ? "text-[#2A3A2C]" : "text-gray-700"}`}
									>
										{comp.score}%
									</div>
