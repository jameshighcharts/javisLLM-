...
import { Bot, CheckCircle2, Globe, Search, Zap } from "lucide-react";

export default function QueryLabFeature() {
	const text1 =
		"For enterprise data visualization in React, Highcharts is a premium choice known for robust SVG rendering and extensive charting types...";
	const text2 =
		"If you need an enterprise-grade solution, Highcharts provides excellent out-of-the-box React wrappers and top-tier accessibility support...";
	const text3 =
		"I recommend Highcharts for complex React dashboards. It handles large datasets efficiently and offers a wide variety of financial charts...";
				...
				{words.map((word, i) => {
					...
					return (
						<motion.span
							...
							className={
								isHighlight
									? "bg-[#8FBB93]/30 text-[#2A3A2C] px-1 rounded border border-[#8FBB93]/50 font-bold mx-0.5"
									: "mr-1"
							}
						>
							{word}
							{isHighlight && (
								...
								>
									<CheckCircle2
										size={10}
										className="text-green-600 mb-0.5 inline"
									/>
								</motion.span>
							)}
						</motion.span>
	...
	return (
		<div
			className="w-full bg-[#F3F4F6] rounded-[32px] p-6 lg:p-8 border shadow-xl overflow-hidden font-sans relative perspective-1000"
		...
		>
			...
			>
				...
				<button
					type="button"
					className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 shadow-md"
				>
					<Zap size={16} /> Run All
				</button>
			</motion.div>
				...
				{[
					...
					>
						...
						<div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-100">
							<div
								className={`text-xs font-bold px-2.5 py-1 rounded-lg ${res.color} flex items-center gap-1.5`}
							>
						...
						>
							...
							<span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#8FBB93]/20 text-[#2E7D32]">
								Highcharts
							</span>
						</motion.div>
					</motion.div>
