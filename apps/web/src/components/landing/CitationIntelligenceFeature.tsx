...
import { Award, Globe, Info, Link2, ShieldCheck } from "lucide-react";

export default function CitationIntelligenceFeature() {
	return (
		<div
			className="w-full bg-[#1A2520] rounded-[32px] p-6 lg:p-8 border shadow-xl overflow-hidden font-sans relative perspective-1000 text-white"
		...
		>
			...
			<div className="flex justify-between items-center mb-8 relative z-10">
				<div>
					...
					<div className="text-sm text-gray-400 mt-1">
						Discover which domains shape LLM answers
					</div>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
						...
						{[
							{
								...
								count: 1842,
								type: "OWNED",
								typeColor: "bg-green-500/20 text-green-400 border-green-500/30",
							},
							{
								domain: "github.com",
								count: 856,
								type: "3RD PARTY",
								typeColor: "bg-gray-500/20 text-gray-300 border-gray-500/30",
							},
							{
							...
							},
							{
								...
								count: 412,
								type: "COMPETITOR",
								typeColor: "bg-red-500/20 text-red-400 border-red-500/30",
							},
						].map((source, idx) => (
							<motion.div
								key={source.domain}
							...
							>
								<div className="flex items-center gap-3">
									...
									<div className="flex flex-col">
										...
										<div
											className={`mt-1 text-[9px] font-bold px-1.5 py-0.5 rounded border inline-flex w-fit ${source.typeColor}`}
										>
											{source.type}
										</div>
									</div>
								...
								<div className="text-right">
									<div className="text-lg font-bold text-[#8FBB93]">
										{source.count}
									</div>
				...
				>
					<div className="text-xs font-bold text-gray-400 tracking-wider mb-5">
						CITATION CONTEXT
					</div>
					...
					<div className="bg-black/30 rounded-xl p-4 border border-white/5 mb-6 shadow-inner relative z-10">
						<div className="text-xs text-gray-400 mb-2 flex items-center gap-1">
							<Info size={12} /> LLM Response Snippet
						</div>
						<p className="text-sm text-gray-300 leading-relaxed font-serif italic">
							"For enterprise dashboards,{" "}
							<span className="bg-[#8FBB93]/20 text-white px-1 rounded border border-[#8FBB93]/30">
								Highcharts
							</span>{" "}
							is often preferred due to its extensive accessibility features and
							export module support [1]."
						</p>
					</div>
					...
					>
						<div className="flex gap-3">
							...
							<div>
								<div className="text-[10px] font-mono text-[#8FBB93] mb-1">
									[1] OWNED DOMAIN
								</div>
								<div className="text-sm font-bold text-white mb-1 line-clamp-1 group-hover:underline">
									Highcharts Accessibility Options
								</div>
								<div className="text-xs text-gray-400 truncate w-full max-w-[200px]">
									highcharts.com/docs/accessibility/accessibility-module
								</div>
							</div>
						</div>
					</motion.div>
