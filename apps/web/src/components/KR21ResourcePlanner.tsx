/* biome-ignore-all lint/a11y/noStaticElementInteractions: planner task surfaces are pointer-driven drag targets */
...
function buildTaskStackLayout(
	tasks: ResourceTask[],
): Map<string, TaskStackLayout> {
	...
	for (const member of TEAM_MEMBERS) {
		...
		const flushCluster = () => {
			...
			for (const task of cluster) {
				...
				const lane =
					freeLanes.length > 0 ? (freeLanes.shift() ?? 0) : laneCursor++;
...
function findSplitInsertDay(
	...
	const rowTasks = sortTasksByStart(
		tasks.filter(
			(task) => task.memberId === targetMemberId && task.id !== movingTaskId,
		),
	);
...
export default function KR21ResourcePlanner({
	tasks: plannerTasks,
	onPatchTask,
}: KR21ResourcePlannerProps) {
	...
	const [splitInsertPreview, setSplitInsertPreview] =
		useState<SplitInsertPreview | null>(null);
	...
	const memberIndex = useMemo(
		() => new Map(TEAM_MEMBERS.map((m, index) => [m.id, index])),
		[],
	);
	...
	const timelineScope = useMemo(() => {
		...
		const scopeEndMs = Math.max(
			minEndMs,
			maxTaskEndMs + EXTRA_SCOPE_WEEKS * 7 * DAY_MS,
		);
		const dayCount = Math.max(
			MIN_TIMELINE_DAYS,
			Math.ceil((scopeEndMs - scopeStartMs) / DAY_MS),
		);
	...
	const mappedPlannerTasks = useMemo<ResourceTask[]>(() => {
		...
		return plannerTasks.map((task) => {
			...
			const startDay = clamp(
				Math.floor((startMs - timelineStartMs) / DAY_MS),
				0,
				timelineDayCount - 1,
			);
			const endDay = clamp(
	...
	const sliderDay = clamp(
		Math.round(sliderValue / DAY_WIDTH),
		0,
		Math.max(0, timelineDayCount - 1),
	);
	const sliderWeek = Math.floor(sliderDay / 7) + 1;
	const sliderDateLabel = formatDateDayMonthFromUtcMs(
		timelineStartMs + sliderDay * DAY_MS,
	);
	...
	const weekGridDays = useMemo(
		() =>
			[
				...new Set([
					...weekSegments.map((segment) => segment.startDay),
					timelineDayCount,
				]),
	...
	const todayDay = useMemo(() => {
		const dayOffset = Math.floor(
			(timelineScope.todayUtcMs - timelineStartMs) / DAY_MS,
		);
	...
	const taskFrames = useMemo(() => {
		...
		for (const task of tasks) {
			...
			const stackLayout = taskStackLayout.get(task.id) ?? {
				lane: 0,
				laneCount: 1,
			};
			const laneCount = Math.max(1, stackLayout.laneCount);
			const laneHeight =
				(rowContentHeight - (laneCount - 1) * LANE_GAP) / laneCount;
			const top =
				row * ROW_HEIGHT +
				ROW_PADDING_Y +
	...
	const selectedTaskPopupPosition = useMemo(() => {
		if (!selectedTaskFrame)
			return { left: 0, top: 0, side: "right" as "right" | "left" };
	...
	const memberStats = useMemo(() => {
		return TEAM_MEMBERS.map((member) => {
			...
			const totalDays = memberTasks.reduce(
				(sum, task) => sum + getTaskDurationDays(task),
				0,
			);
			const conflicts = memberTasks.filter((task) =>
	...
	useEffect(() => {
		...
		const onPointerMove = (event: PointerEvent) => {
			...
			const minEnd =
				dragState.mode === "resize-left" ? dragState.originEndDay - 1 : 1;
		...
		const onPointerUp = (event: PointerEvent) => {
			...
			if (!hasMoved && dragState.mode === "move") {
				setSelectedTaskId((current) =>
					current === dragState.taskId ? null : dragState.taskId,
				);
			} else {
				const baselineById = new Map(
					dragState.originTasks.map((task) => [task.id, task]),
				);
				for (const committed of tasksRef.current) {
					...
					onPatchTask(committed.sourceTaskId, {
						...
						start: toIsoDateFromUtcMs(
							timelineStartMs + committed.startDay * DAY_MS,
						),
						end: toIsoDateFromUtcMs(
							timelineStartMs + committed.endDay * DAY_MS,
						),
					});
				}
			}
	...
	function openMember(
		...
		const avatar = event.currentTarget.querySelector(
	...
	return (
		<section
			...
			style={{
				borderColor: "#DDD0BC",
				background: `radial-gradient(circle at 8% 10%, ${rgba("#8FBB93", 0.1)}, transparent 32%), radial-gradient(circle at 90% 0%, ${rgba("#C8A87A", 0.08)}, transparent 30%), ${THEME.timelineBg}`,
			}}
		>
			<div
									...
									<span className="flex items-center gap-3">
										...
										<span className="min-w-0 flex-1">
											<span
												...
												style={{ color: THEME.timelineText }}
											>
												{member.name}
											</span>
										</span>
						...
						>
							<div className="relative" style={{ width: timelineWidth }}>
								...
								>
											...
											<div className="flex flex-col items-center leading-tight">
												...
												>
													{formatDateShortFromUtcMs(
														timelineStartMs + segment.startDay * DAY_MS,
													)}{" "}
													-{" "}
													{formatDateShortFromUtcMs(
														timelineStartMs + (segment.endDay - 1) * DAY_MS,
													)}
												</span>
											</div>
										</div>
									...
									>
										Today ·{" "}
										{formatDateDayMonthFromUtcMs(timelineScope.todayUtcMs)} · W
										{todayWeekNumber}
									</div>
								</div>
								...
								>
									{TEAM_MEMBERS.map((member, rowIndex) => (
										<div
											key={member.id}
									...
									{splitInsertPreview ? (
										<div
											...
											style={{
												...
												top:
													(memberIndex.get(splitInsertPreview.memberId) ?? 0) *
														ROW_HEIGHT +
													8,
												width: 2,
												...
												background: "#E45F3E",
												boxShadow:
													"0 0 0 4px rgba(228,95,62,0.15), 0 0 14px rgba(228,95,62,0.35)",
												borderRadius: 999,
												zIndex: 8,
											}}
										/>
										...
										return (
											...
											>
												...
												<button
													...
													onPointerDown={(event) =>
														beginDrag(task, "resize-left", event)
													}
													style={{
														position: "absolute",
														left: 4,
												...
												<button
													...
													onPointerDown={(event) =>
														beginDrag(task, "resize-right", event)
													}
													style={{
														position: "absolute",
														right: 4,
									...
									{selectedTask && selectedTaskFrame && !dragState ? (
										<div
											...
											style={{
												...
												boxShadow:
													"0 8px 32px rgba(30,42,32,0.14), 0 2px 8px rgba(30,42,32,0.08)",
												zIndex: 10,
												pointerEvents: "auto",
											}}
										>
											{/* Colour accent strip */}
											<div
												style={{ height: 3, background: selectedTask.color }}
											/>
													...
													<>
														...
														>
															{selectedTask.subtasks.map((subtask, index) => {
																const checked =
																	selectedTaskChecks[index] ?? false;
																return (
																	<li
																		key={`${selectedTask.id}-${subtask}`}
				...
				<div className="absolute inset-0 z-20">
					...
					<div
						...
						style={{
							...
							transition:
								"opacity 320ms ease, transform 420ms cubic-bezier(0.2,0.8,0.2,1)",
						}}
					>
						<button
							type="button"
						...
						{focusAnchor ? (
							<img
								...
								style={{
									...
									boxShadow: "0 8px 20px rgba(42,58,44,0.14)",
									transition:
										"left 420ms cubic-bezier(0.2,0.8,0.2,1), top 420ms cubic-bezier(0.2,0.8,0.2,1), width 420ms cubic-bezier(0.2,0.8,0.2,1), height 420ms cubic-bezier(0.2,0.8,0.2,1)",
								}}
							/>
						) : null}
							...
							>
								<div className="mb-4 flex items-center justify-between">
									...
									<span
										...
										style={{ color: THEME.detailMuted }}
									>
										{focusedTasks.length} card
										{focusedTasks.length === 1 ? "" : "s"}
									</span>
								</div>
										...
										{focusedTasks.map((task) => {
											...
											return (
												...
												>
													...
													>
														{formatDateShortFromUtcMs(
															timelineStartMs + task.startDay * DAY_MS,
														)}{" "}
														-{" "}
														{formatDateShortFromUtcMs(
															timelineStartMs + task.endDay * DAY_MS,
														)}
													</p>
