/* biome-ignore-all lint/a11y/useKeyWithClickEvents: custom modal backdrops in the shell are intentional */
		...
		icon: (
			...
			>
				<path
					strokeLinecap="round"
					strokeLinejoin="round"
					d="M10 2v4l-5 8a4 4 0 003.38 6h7.24A4 4 0 0019 14l-5-8V2"
				/>
...
export default function Layout({ children }: { children: ReactNode }) {
	...
	const [competitorsDesktopOpen, setCompetitorsDesktopOpen] =
		useState(onCompetitorsPath);
	const [competitorsMobileOpen, setCompetitorsMobileOpen] =
		useState(onCompetitorsPath);
	const videoRef = useRef<HTMLVideoElement>(null);
	const [headerExtra, setHeaderExtraRaw] = useState<ReactNode>(null);
	const setHeaderExtra = useCallback(
		(node: ReactNode) => setHeaderExtraRaw(node),
		[],
	);
		...
		<HeaderExtraContext.Provider value={setHeaderExtra}>
			<div className="flex h-[100dvh] overflow-hidden">
				...
				>
					...
					>
						{/* biome-ignore lint/a11y/useMediaCaption: teaser clip is decorative and has no spoken dialogue */}
						<video
							ref={videoRef}
							src={videoSrc}
							poster="/app-icon.jpg"
					...
					>
						...
						<nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
							{NAV.map((item) => {
								...
								if (item.children) {
									const isOpen = competitorsMobileOpen;
									const setOpen = setCompetitorsMobileOpen;
									return (
				...
				>
					...
					<nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
						{NAV.map((item) => {
							...
							if (item.children) {
								const isOpen = competitorsDesktopOpen;
								const setOpen = setCompetitorsDesktopOpen;
								return (
									<div key={item.to} className="space-y-0.5">
										<button
											type="button"
				...
				<div className="flex-1 flex flex-col min-w-0 overflow-hidden">
					...
					<main
						className="flex-1 overflow-y-auto px-3 py-4 sm:px-6 sm:py-5 pb-28 md:pb-5"
