...
function ensureOpenAiModel(model) {
	...
	if (provider !== "openai") {
		const error = new Error(
			"Research endpoints currently require an OpenAI model for web search.",
		);
		error.statusCode = 400;
		throw error;
	}
...
function extractMetaDescription(html) {
	...
	return reverseMatch && reverseMatch[1]
		? decodeXmlEntities(reverseMatch[1].trim())
		: "";
}
...
async function runOpenAiJsonWithRetry({
	model,
	systemPrompt,
	userPrompt,
	maxAttempts = 2,
...
function addWeeks(isoDate, weeks) {
	...
	return new Date(
		parsed + Math.max(0, Number(weeks) || 0) * 7 * 24 * 60 * 60 * 1000,
	).toISOString();
}
