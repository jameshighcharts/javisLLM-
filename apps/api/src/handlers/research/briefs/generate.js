...
function buildBriefPrompt(gapRow) {
	const evidence = Array.isArray(gapRow.evidence_citations)
		? gapRow.evidence_citations
		: [];
	...
	try {
		...
		if (!hasNonEmptyCitationArray(llmResult.parsed?.brief_citations)) {
			const error = new Error(
				"Generated brief must include non-empty citations.",
			);
			error.statusCode = 400;
			throw error;
		}
		const briefCitations = normalizeBriefCitations(
			llmResult.parsed?.brief_citations,
		);
		...
		if (!hasAllRequiredSections(briefMarkdown)) {
			const error = new Error(
				"Generated brief missing one or more required sections.",
			);
			error.statusCode = 502;
			throw error;
		}
