...
function gatherEvidenceForPrompt(queryText, normalizedPosts) {
	...
	for (const post of normalizedPosts) {
		const haystack =
			`${post.title} ${post.summary} ${post.theme}`.toLowerCase();
		let matches = 0;
...
module.exports = async (req, res) => {
	...
	try {
		...
		const [runs, competitors, promptQueries, blogPosts, existingGaps] =
			await Promise.all([
				supabaseRestRequest(
					config,
		...
		if (!latestRun?.id) {
			const error = new Error(
				"No completed benchmark run found. Run a benchmark before refreshing gaps.",
			);
		...
		const brandCompetitor =
			(Array.isArray(competitors) ? competitors : []).find(
				(entry) => entry?.is_primary,
			) ||
			(Array.isArray(competitors) ? competitors : []).find(
				(entry) => cleanText(entry?.slug).toLowerCase() === "highcharts",
			) ||
			null;

		if (!brandCompetitor?.id) {
			const error = new Error(
				"Could not identify primary brand competitor (is_primary or slug=highcharts).",
			);
			error.statusCode = 400;
			throw error;
		}
		...
		for (const prompt of Array.isArray(promptQueries) ? promptQueries : []) {
			...
			const brandRatePct =
				rates.find((entry) => entry.competitorId === brandCompetitor.id)
					?.ratePct || 0;
		...
		const stats = {
			runId: latestRun.id,
			promptsConsidered: Array.isArray(promptQueries)
				? promptQueries.length
				: 0,
