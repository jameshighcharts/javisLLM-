...
module.exports = async (req, res) => {
	...
	try {
		...
		for (const competitor of Array.isArray(competitors) ? competitors : []) {
			...
			const knownLinks =
				knownLinksBySource.get(competitorName.toLowerCase()) || [];
			const domainCandidates = inferDomainCandidates(
				competitorName,
				knownLinks,
			);
			...
			if (normalizedItems.length === 0) {
				const error = new Error(
					`No valid research items returned for ${competitorName}.`,
				);
			...
			for (const item of normalizedItems) {
				...
				rowsToUpsert.push({
					...
					source_slug:
						competitorSlug ||
						competitorName.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
					title: item.title,
