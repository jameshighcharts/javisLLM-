...
async function waitForPromptInput(
	...
	try {
		...
		const loginLink = page
			.locator(
				'a[href*="auth/login"], a[href*="/login"], button:has-text("Log in")',
			)
			.first();
...
async function waitForAssistantTurn(
	...
	throw buildHttpError(
		"Timed out waiting for assistant response to start.",
		504,
	);
}
...
async function waitForResponseToSettle(
	...
	const assistantTurn = page
		.locator(ASSISTANT_TURN_SELECTOR)
		.nth(assistantIndex);
	...
	throw buildHttpError(
		"Timed out waiting for assistant response to finish streaming.",
		504,
	);
}
		...
		({ assistantSelector, assistantTurnIndex }) => {
			...
			if (assistantTurn) {
				for (const anchor of Array.from(
					assistantTurn.querySelectorAll("a[href]"),
				)) {
					pushAnchor(anchor);
				}
			}
...
export async function runChatGptWebPromptLabQuery(options: {
	...
	try {
		...
		await page.goto(CHATGPT_WEB_URL, {
		...
		const assistantIndex = await waitForAssistantTurn(
			page,
			assistantCountBefore,
			timeoutMs,
		);
		const responseText = await waitForResponseToSettle(
			page,
			assistantIndex,
			timeoutMs,
		);
		...
		const citationCandidates = await extractCitationCandidates(
			page,
			assistantIndex,
		);
		const citationRefs = normalizeChatGptWebCitationRefs(
			citationCandidates,
			responseText,
		);
