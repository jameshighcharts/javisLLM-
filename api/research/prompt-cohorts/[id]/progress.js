...
async function fetchAverageMentionRateForRun(config, runId, promptIds, competitorId) {
...
module.exports = async (req, res) => {
  try {
    ...
    const primaryCompetitor = (Array.isArray(competitorRows) ? competitorRows : []).find((row) => row?.is_primary)
      || (Array.isArray(competitorRows) ? competitorRows : []).find((row) => cleanText(row?.slug).toLowerCase() === 'highcharts')
      || null
    ...
    const baselineRate = baselineMetric === null ? null : Number(baselineMetric.toFixed(2))
    const currentRate = currentMetric === null ? null : Number(currentMetric.toFixed(2))
    const upliftPp =
    ...
    return sendJson(res, 200, {
      ...
      upliftPp,
      progressPct,
      dueDate: addWeeks(cohort.baseline_locked_at, toNumber(cohort.target_weeks, 8)),
      trend,
    })
  } catch (error) {
    const statusCode =
