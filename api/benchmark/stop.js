...
async function resolveTargetRunId(config, explicitRunId) {
  if (explicitRunId) {
    ...
    if (!first || typeof first !== 'object' || !cleanRunId(String(first.id || ''))) {
  ...
  const first = Array.isArray(activeProgressRows) ? activeProgressRows[0] : null
  const runId = first && typeof first === 'object' ? cleanRunId(String(first.run_id || '')) : ''
  if (!runId) {
...
async function stopQueueRun(config, runId) {
  ...
  const cancelledJobCount = Array.isArray(cancelledRows) ? cancelledRows.length : 0
...
module.exports = async (req, res) => {
  try {
    ...
    const rateLimitWindowMs = Number(process.env.BENCHMARK_STOP_RATE_WINDOW_MS || 60 * 1000)
    ...
    if (!isQueueTriggerEnabled()) {
      const error = new Error('Stop is only supported when USE_QUEUE_TRIGGER=true.')
