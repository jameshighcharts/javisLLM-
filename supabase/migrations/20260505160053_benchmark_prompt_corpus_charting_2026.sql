-- Replace the active benchmark prompt corpus with the first charting search-intent batch.
-- Historical benchmark rows are preserved by keeping old prompt rows and marking them inactive/deleted.

with new_prompts(query_text, sort_order, tags) as (
  values
...
set is_active = false,
    tags = case
      when '_deleted' = any(pq.tags) then pq.tags
      else array_append(pq.tags, '_deleted')
    end
where not exists (
  select 1
  from new_prompts as np
  where np.query_text = pq.query_text
);
