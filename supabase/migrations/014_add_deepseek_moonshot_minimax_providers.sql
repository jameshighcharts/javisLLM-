-- Add DeepSeek, Moonshot/Kimi, and MiniMax provider inference for queue jobs and owner rollups.
...
begin
  ...
  if v_model like 'deepseek%' then
    return 'DeepSeek';
  end if;
  if v_model like 'kimi%' or v_model like 'moonshot/%' then
    return 'Moonshot AI';
  end if;
  if v_model like 'minimax%' then
    return 'MiniMax';
  end if;
  return 'Unknown';
end;
...
create or replace function public.enqueue_benchmark_run(
  ...
  p_prompt_order text default 'default'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_prompt_order text := nullif(lower(trim(coalesce(p_prompt_order, ''))), '');
...
begin
  ...
  if v_prompt_order is null then
    v_prompt_order := 'default';
  end if;
  if v_prompt_order not in ('default', 'newest') then
    raise exception 'p_prompt_order must be "default" or "newest"';
  end if;
  ...
  for v_prompt in
    ...
    order by
      case when v_prompt_order = 'newest' then 0 else 1 end,
      case when v_prompt_order = 'newest' then pq.created_at end desc nulls last,
      case when v_prompt_order = 'newest' then pq.updated_at end desc nulls last,
      case when v_prompt_order = 'newest' then pq.id end desc,
      case when v_prompt_order <> 'newest' then pq.sort_order end asc,
      case when v_prompt_order <> 'newest' then pq.created_at end asc,
      pq.id asc
  ...
  loop
    ...
    loop
      v_provider := case
        ...
        when lower(v_model) like 'deepseek%' then 'deepseek'
        when lower(v_model) like 'kimi%' or lower(v_model) like 'moonshot/%' then 'moonshot'
        when lower(v_model) like 'minimax%' then 'minimax'
        else 'openai'
      end;
  ...
  return jsonb_build_object(
    ...
    'prompt_order', v_prompt_order
  );
end;
$$;

revoke all on function public.enqueue_benchmark_run(uuid, text[], text, int, numeric, boolean, int, text, text) from public;
grant execute on function public.enqueue_benchmark_run(uuid, text[], text, int, numeric, boolean, int, text, text) to authenticated, service_role;
