...
ROOT_DIR = REPO_ROOT
DEFAULT_CONFIG_PATH = ROOT_DIR / "config" / "benchmark" / "config.json"
...
def sync_config(client: Client, config: Dict[str, Any]) -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    execute_or_raise(
        ...
        .upsert(cast(Any, competitor_rows), on_conflict="slug")
        .execute(),
...
def sync_run_data(
    ...
    run_rows = execute_or_raise(
        client.table("benchmark_runs").insert(cast(Any, run_payload)).execute(),
    ...
    for record in jsonl_rows:
        ...
        mention_map = (
            cast(Dict[str, Any], record.get("mentions"))
            if isinstance(record.get("mentions"), dict)
            else {}
        )
