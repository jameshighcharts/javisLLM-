...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if prompt_error and is_missing_column_error(prompt_error):
        ...
        prompt_error = getattr(prompt_result, "error", None)
    if prompt_error:
        raise SyncError(f"Failed to read prompt_queries: {prompt_error}")
    ...
    competitor_error = getattr(competitor_result, "error", None)
    if competitor_error:
        raise SyncError(f"Failed to read competitors: {competitor_error}")
